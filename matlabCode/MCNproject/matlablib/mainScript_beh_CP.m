...
p1 = 0.2;
p2 = 0.9;
threshTest = 0.4:0.02:0.65;
sim = 150;
test = cell(length(lenTest),length(threshTest),sim);

%%
    for j = 1:length(lenTest)
        for i = 1:length(threshTest)
            currThresh = threshTest(i);
            for s = 1:sim
                rewardProbs = taskSimulate(p1, p2, 500, [lenTest(j), lenTest(j)],0);
                beh = taskPerformCPSigma(rewardProbs,0,'swThresh',currThresh);
                test{j,i,s} = beh;
            end
        end
    end
...
figure;
errorbar(lenTest, sigmaMean, sigmaSem, 'color', 'm', 'linewidth', 2);
title('mean sigma')
%%
colors = cool(length(threshTest));
figure; hold on;
for h = 1:length(lenTest)
    ...
    legend(cellfun(@num2str,num2cell(threshTest), 'UniformOutput', false))
end
    ...
    for j = 1:length(sigmaTest)
        ...
        for s = 1:sim
            ...
            for sw = 1:length(allSwitches)
                ...
                if currBeh.rewardsP(allSwitches(sw),2) < currBeh.rewardsP(allSwitches(sw)-1,2)
                    swCs = [swCs; currSwCs];
                    swCB = [swCB; currConfBar];
                    swSp = [swSp; currSup];
                    swSigma = [swSigma; currSigma];
                else
                    swCs = [swCs; 1-currSwCs];
                    swCB = [swCB; currConfBar];
                    swSp = [swSp; currSup];
                    swSigma = [swSigma; currSigma];
                end
...
rwdRates = zeros(length(lenTest), length(threshTest), sim);
...
hold on;
for j = 1:length(lenTest)
    bar =  std(squeeze(rwdRates(j,:,:)), 0, 2)/sqrt(sim-1);
    m = mean(squeeze(rwdRates(j,:,:)),2);
    plot(threshTest, m, 'color', colors(j,:), 'linewidth',2);

    fill([threshTest, flip(threshTest)], [m-bar; flip(m+bar)], colors(j,:),'FaceAlpha', 0.2,'EdgeColor', 'none' )
