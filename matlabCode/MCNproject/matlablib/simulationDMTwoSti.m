function s = simulationDMTwoSti(p,I0,I1,I2,thresh,dmOnline)
% I1 and I2 are sensory input; e.g. 0 0125, 0 0083 for evidence
% accumulation; 0.33 0.27 for matching
delay = 0/1000;
NT = round(p.Ttotal/p.dt);
t_plot = [1:NT] * p.dt;
t_stim = (t_plot > p.Tstim_on+delay) & (t_plot < p.Tstim_off);
%t_app = (t_plot > p.Tstim_on) & (t_plot < p.Tstim_on + 0.3);
tDecay = t_plot(t_plot > p.Tstim_on & t_plot < p.Tstim_off);
tDecay = tDecay - tDecay(1);
Iapp = 0.5*ones(size(t_plot));
Iapp(t_plot > p.Tstim_on & t_plot < p.Tstim_off) = 6*exp(-tDecay/0.12)+1; 

%mean_stim = ones(1,NT) * p.mu0 * p.Jext/1000; %[nA]
%diff_stim = p.Jext * p.mu0 * coh/100. * 2;

Istim1_plot = I1 .* t_stim + I0 * Iapp; %[nA]
Istim2_plot = I2 .* t_stim + I0 * Iapp;

% filter input
x = 1:200;
myfilter = exp(-x/20).*sin(0.04*x);
myfilter = myfilter/sum(myfilter);
Istim1_plot = [Istim1_plot(1)*ones(1,length(myfilter)-1), Istim1_plot];
Istim2_plot = [Istim2_plot(1)*ones(1,length(myfilter)-1), Istim2_plot];
Istim1_plot = conv(Istim1_plot,myfilter,'same');
Istim2_plot = conv(Istim2_plot,myfilter,'same');
Istim1_plot = Istim1_plot(1:length(t_plot));
Istim2_plot = Istim2_plot(1:length(t_plot));
...
for i_t = 1:NT
    ...
    r1  = FIcurve(Isyn1,'a',p.a);
    r2  = FIcurve(Isyn2,'a',p.a);
