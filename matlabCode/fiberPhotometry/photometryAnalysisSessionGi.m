function photometryAnalysisSessionGi(session, modelName, category, tag)
% load data
[root, sep] = currComputer();
pd = parseSessionString_df(session, root, sep);
if ~isempty(tag)>0
    dataPath = [pd.sortedFolder session '_photometryGi_' tag '.mat'];
else
    dataPath = [pd.sortedFolder session '_photometryGi.mat'];
numSamps = 2000;
numBins = 4;
psthBinNum = 4;
...
THmatChoiceFilteredG = [];
mPFCmatChoiceFilteredG = [];
GimatChoiceFilteredG = [];

for j = 1:size(mPFCmatChoiceG,1)
    temp = conv(THmatChoiceG(j,:), myKernel);
    temp = temp((floor(0.5*length(myKernel))+1):(end-floor(0.5*length(myKernel))));
    THmatChoiceFilteredG(j,:) = temp;

    temp = conv(GimatChoiceG(j,:), myKernel);   
    temp = temp((floor(0.5*length(myKernel))+1):(end-floor(0.5*length(myKernel))));
    GimatChoiceFilteredG(j,:) = temp;
...
for c  = 1
    ...
    for i = 1:length(names)
        ...
        saveFigurePDF(currfig,[savePath session '_' names{i} '_' currC '_' tag '.pdf']);
    end
end
end