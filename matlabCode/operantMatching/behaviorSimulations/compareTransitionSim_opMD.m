function compareTransitionSim_opMD(xlFile, sheet, category, beh, varargin)
...
p.addParameter('runs', 10000);
p.addParameter('randomSeed', 587);
p.addParameter('modelName', 'sixParam_absPePeAN_bi');
p.addParameter('modelCategory', 'clean');
...
[wsls, transLow, transHigh] = transitionAnalysis_opMD(xlFile, sheet, category, [90 50 10], tranWin);
...
for currSesh = 1:length(dayList)
    ...
    if strcmp(animal, prevAnimal) == 0
        ...
        fracSesh = sum(~cellfun(@isempty, regexp(dayList, animal))) / length(dayList);
        ...
        for i = 1:runs
            ...
            switch p.Results.modelName
                ...
                case 'sixParam_absPePeAN_biSep'
                    [~, allRewards, allChoices, blockProbs, blockSwitch] = qLearningModel_absPePeAN_biSep_simNoPlot('params', params,...
                        'maxTrials', p.Results.maxTrials, 'randomSeed', rSeed, 'rwdProbs', p.Results.rwdProbs);
                case 'sevenParam_absPePeAN_biSep_f'
                    [~, allRewards, allChoices, blockProbs, blockSwitch] = qLearningModel_absPePeAN_biSep_f_simNoPlot('params', params,...
                        'maxTrials', p.Results.maxTrials, 'randomSeed', rSeed, 'rwdProbs', p.Results.rwdProbs);
                case 'sevenParam_absPePeAN_bi_k'
                    [~, allRewards, allChoices, blockProbs, blockSwitch] = qLearningModel_absPePeAN_bi_k_simNoPlot('params', params,...
                        'maxTrials', p.Results.maxTrials, 'randomSeed', rSeed, 'rwdProbs', p.Results.rwdProbs); 
                ...
                case 'sevenParam_absPePeLR'
                    [~, allRewards, allChoices, blockProbs, blockSwitch] = qLearningModel_absPePeLR_simNoPlot('params', params,...
                        'maxTrials', p.Results.maxTrials, 'randomSeed', rSeed, 'rwdProbs', p.Results.rwdProbs);
                case 'eightParam_absPePeLR_k'
                    [~, allRewards, allChoices, blockProbs, blockSwitch] = qLearningModel_absPePeLR_k_simNoPlot('params', params,...
                        'maxTrials', p.Results.maxTrials, 'randomSeed', rSeed, 'rwdProbs', p.Results.rwdProbs);
...
lowSem = sem_bernoulli(sum(transLow==1), repmat(size(transLow,1), 1, range*2));
highAvg = mean(transHigh,1);
highSem = sem_bernoulli(sum(transHigh==1), repmat(size(transHigh,1), 1, range*2));

lowAvgSim = mean(transLowSim,1);
lowSemSim = sem_bernoulli(sum(transLowSim==1), repmat(size(transLowSim,1), 1, range*2));
highAvgSim = mean(transHighSim,1);
highSemSim = sem_bernoulli(sum(transHighSim==1), repmat(size(transHighSim,1), 1, range*2));
...
x = [-range+1:range];
errorbar(x, lowAvg, lowSem, '-', 'Color', colors(1,:), 'linewidth', 1.3)
errorbar(x, highAvg, highSem, '-', 'Color', colors(3,:), 'linewidth', 1.3)
...
errorbar(x, lowAvgSim, lowSemSim, '-', 'Color', colors(2,:), 'linewidth', 1.3)
errorbar(x, highAvgSim, highSemSim, '-', 'Color', colors(4,:), 'linewidth', 1.3)
...
errorbar(x, wsls.lS_high, wsls.sem_lS_low, 'Color', colors(3,:), 'linewidth', 1.3)
ylabel('Lose-shift')
xlabel('Trials from switch')
legend('medium -> low', 'high -> low')
yls = ylim;
...
errorbar(x, wS_low, sem_wS_low, 'Color', colors(2,:), 'linewidth', 1.3)
errorbar(x, wS_high, sem_wS_high, 'Color', colors(4,:), 'linewidth', 1.3)
ylabel('Win-stay')
xlabel('Trials from switch')
ylim([yws])
plot([0 0], yws, '--k')
...
errorbar(x, lS_low, sem_lS_low, 'Color', colors(2,:), 'linewidth', 1.3)
errorbar(x, lS_high, sem_lS_low, 'Color', colors(4,:), 'linewidth', 1.3)
ylabel('Lose-shift')
xlabel('Trials from switch')
legend('sim: medium -> low', 'sim: high -> low')
ylim([yls])
