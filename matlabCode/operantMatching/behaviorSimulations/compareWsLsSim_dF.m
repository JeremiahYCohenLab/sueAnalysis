function compareWsLsSim_dF(xlFile, sheet, category, beh, varargin)
...
[wsRwdHx, lsRwdHx, wS, lS] = wslsRwdCount_dF(xlFile, sheet, category, 'tMax', p.Results.tMax, 'revForFlag', p.Results.revForFlag);

%initialize structures for simulated ws-ls
wsSim = cell(1, numMdls);
lsSim = cell(1, numMdls);
...
for currSesh = 1:length(dayList)
    ...
    if strcmp(animal, prevAnimal) == 0 || p.Results.sessionParamsFlag
        ...
        for currM = 1:numMdls
            ...
            wsSim{currM} = [wsSim{currM}; mean(wsTmp)];
            lsSim{currM} = [lsSim{currM}; mean(lsTmp)];
            wsRwdHxSim{currM} = [wsRwdHxSim{currM}; nanmean(wsRwdHxTmp,1)];
            lsRwdHxSim{currM} = [lsRwdHxSim{currM}; nanmean(lsRwdHxTmp,1)];
        end
    end 
end

if p.Results.sessionParamsFlag
    wsSimTmp = wsSim; wsSim = cell(1, numMdls);
    lsSimTmp = lsSim; lsSim = cell(1, numMdls);
    wsRwdHxSimTmp = wsRwdHxSim; wsRwdHxSim = cell(1, numMdls);
    lsRwdHxSimTmp = lsRwdHxSim; lsRwdHxSim = cell(1, numMdls);
    for currA = 1:length(animals)
        aI = find(~cellfun(@isempty, regexp(dayList, animals{currA})));
        for currM = 1:numMdls
            wsSim{currM} = [wsSim{currM}; mean(wsSimTmp{currM}(aI))];
            lsSim{currM} = [lsSim{currM}; mean(lsSimTmp{currM}(aI))];
            wsRwdHxSim{currM} = [wsRwdHxSim{currM}; nanmean(wsRwdHxSimTmp{currM}(aI,:), 1)];
            lsRwdHxSim{currM} = [lsRwdHxSim{currM}; nanmean(lsRwdHxSimTmp{currM}(aI,:), 1)];
        end
    end
end
