function [t, allRewards, allChoices, blockProbs, blockSwitch, laser] = qLearningModel_5params_k_bias_LaserNegOnlyRPE_simNoPlot(params, maxTrial, randomSeed, varargin)
%
%
...
for currT = 1:maxTrial
    ...
    if binornd(1, pRight(currT)) == 0 % left choice selected probabilistically
        ...
        if laser(currT) == 1 && abs(allRewards(currT))==0
            rpe = rpe + diff;
        end
        pe(currT) = rpe;
