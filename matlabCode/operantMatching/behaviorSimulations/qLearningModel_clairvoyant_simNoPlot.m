function [allRewards, allChoices, blockProbs, blockSwitch] = qLearningModel_clairvoyant_simNoPlot(varargin)
%
%
...
for currT = 1:p.MaxTrials - 1    
    ...
    if p.RewardProbabilities(1,1) >= p.RewardProbabilities(1,2)
        p.inputChoice([1 0]);
        allChoices(currT) = -1;
        allRewards(currT) = p.AllRewards(currT, 1) * -1;
    elseif p.RewardProbabilities(1,2) > p.RewardProbabilities(1,1)
        p.inputChoice([0 1]);
        allChoices(currT) = 1;
