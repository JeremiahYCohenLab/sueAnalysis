function [rBar, allRewards, allChoices, blockProbs, blockSwitch] = qLearningModel_peBeta_avg_sim(varargin)
%
%
...
for currT = 1:p.MaxTrials - 1
    ...
    % Select action
    pRight = 1/(1 + exp(-beta(currT)*diff(Q(currT, :))));
    subplot(qValue_plot)
    title(sprintf('Probability of right choice %d%%', round(pRight*100)))
    ...
    rBar(currT + 1) = v * abs(rpe) + (1-v) * rBar(currT);
...
suptitle('Q rBar -> beta model simulated behavior');
