function [t, allRewards, allChoices, blockProbs, blockSwitch, ITI] = qLearningModel_absPePeAN_biSep_f_simNoPlot(varargin)
%
%
...
peBar_L = [0; NaN(a.Results.maxTrials-1, 1)];
peBar_R = [0; NaN(a.Results.maxTrials-1, 1)];
...
for currT = 1:p.MaxTrials - 1
    ...
    if binornd(1, pRight) == 0 % left choice selected probabilistically
        ...
        pePe(currT) = abs(rpe(currT)) - peBar_L(currT);
        ...
        peBar_L(currT + 1) = peBar_L(currT) + aPE * pePe(currT);
        peBar(currT) = peBar_L(currT);
        peBar_R(currT + 1) = peBar_R(currT) * aFpeBar;
        if peBar_R(currT + 1) > 1
            peBar_R(currT + 1) = 1;
        end
    else
        p.inputChoice([0 1]);
        ...
        pePe(currT) = abs(rpe(currT)) - peBar_R(currT);
        aN = pePe(currT) * aNscale + aNmin;
        if rpe(currT) >= 0
        ...
        peBar_R(currT + 1) = peBar_R(currT) + aPE * pePe(currT);
        peBar(currT) = peBar_R(currT);
        peBar_L(currT + 1) = peBar_L(currT) * aFpeBar;
        if peBar_L(currT + 1) > 1
            peBar_L(currT + 1) = 1;
        end
    end
end
