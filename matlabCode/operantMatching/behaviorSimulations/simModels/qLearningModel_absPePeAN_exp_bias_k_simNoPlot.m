function [peBar, allRewards, allChoices, blockProbs, blockSwitch] = qLearningModel_absPePeAN_exp_bias_k_simNoPlot(varargin)
%
%
...
for currT = 1:p.MaxTrials - 1
    ...
    if binornd(1, pRight) == 0 % left choice selected probabilistically
        ...
        pePe(currT) = abs(rpe) - peBar(currT);
        aN = aNa * exp(pePe(currT) * aNb);
        if rpe >= 0
