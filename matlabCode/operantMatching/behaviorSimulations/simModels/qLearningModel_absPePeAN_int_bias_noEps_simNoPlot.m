function [t, allRewards, allChoices, blockProbs, blockSwitch, ITI] = qLearningModel_absPePeAN_int_bias_noEps_simNoPlot(varargin)
%
%
...
for currT = 1:p.MaxTrials - 1
    ...
    if binornd(1, pRight) == 0 % left choice selected probabilistically
        ...
        if rpe(currT) >= 0
            ...
            Q(currT + 1, 1) = Q(currT, 1) + aN(currT + 1)*rpe(currT);
        end
        ...
        if rpe(currT) >= 0
            ...
            Q(currT + 1, 2) = Q(currT, 2) + aN(currT + 1)*rpe(currT);
        end
