function [t, allRewards, allChoices, blockProbs, blockSwitch, ITI] = qLearningModel_absPePeAN_log3_simNoPlot(varargin)
%
%
...
aNc = a.Results.params(3);
aP = a.Results.params(4);
...
for currT = 1:p.MaxTrials - 1
    ...
    if binornd(1, pRight) == 0 % left choice selected probabilistically
        ...
        aN = logistic((pePe(currT) + aNa) * aNb) * aNc;
        if aN < 0
            aN = 0;
        end
        if rpe(currT) >= 0
