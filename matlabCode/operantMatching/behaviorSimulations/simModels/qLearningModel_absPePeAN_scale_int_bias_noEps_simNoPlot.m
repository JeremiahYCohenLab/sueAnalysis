function [t, allRewards, allChoices, blockProbs, blockSwitch, ITI] = qLearningModel_absPePeAN_scale_int_bias_noEps_simNoPlot(varargin)
%
%
...
aNmin = a.Results.params(1);
aP = a.Results.params(2);
aF = a.Results.params(3);
aPE = a.Results.params(4);
v = a.Results.params(5);
beta = a.Results.params(6);
if length(a.Results.params) == 7
    bias = a.Results.params(7);
else
    bias = 0;
end
...
aN = [aNmin zeros(1, a.Results.maxTrials-1)];
for currT = 1:p.MaxTrials
    ...
    if binornd(1, pRight) == 0 % left choice selected probabilistically
        ...
        if rpe(currT) >= 0
            Q(currT + 1, 1) = Q(currT, 1) + aP*rpe(currT)* (1 - peBar(currT));
            aN(currT + 1) = aN(currT);
        else
            aN(currT + 1) = v * (pePe(currT) + aNmin) + (1 - v) * aN(currT);
            if aN(currT + 1) < 0
                aN(currT + 1) = 0;
            end
            Q(currT + 1, 1) = Q(currT, 1) + aN(currT + 1)*rpe(currT) * (1 - peBar(currT));
        end
        ...
        pePe(currT) = abs(rpe(currT)) - peBar(currT);
        if rpe(currT) >= 0
            Q(currT + 1, 2) = Q(currT, 2) + aP*rpe(currT)* (1 - peBar(currT));
            aN(currT + 1) = aN(currT);
        else
            aN(currT + 1) = v * (pePe(currT) + aNmin) + (1 - v) * aN(currT);
            if aN(currT + 1) < 0
            ...
            Q(currT + 1, 2) = Q(currT, 2) + aN(currT + 1)*rpe(currT) * (1 - peBar(currT));
        end
