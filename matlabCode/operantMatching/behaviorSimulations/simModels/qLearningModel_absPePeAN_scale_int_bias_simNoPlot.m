function [t, allRewards, allChoices, blockProbs, blockSwitch, ITI] = qLearningModel_absPePeAN_scale_int_bias_simNoPlot(varargin)
%
%
...
for currT = 1:p.MaxTrials - 1
    ...
    if binornd(1, pRight) == 0 % left choice selected probabilistically
        ...
        if rpe(currT) >= 0
            ...
            aN = v * (pePe(currT) * aNscale + aNmin) + (1 - v) * aN;
            if aN < 0
                aN = 0;
            end
            Q(currT + 1, 1) = Q(currT, 1) + aN*rpe(currT) * (1 - peBar(currT));
        end
        ...
        if rpe(currT) >= 0
            ...
            Q(currT + 1, 2) = Q(currT, 2) + aN*rpe(currT) * (1 - peBar(currT));
        end
