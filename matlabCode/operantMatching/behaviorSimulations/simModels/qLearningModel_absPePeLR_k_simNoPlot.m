function [peBar, allRewards, allChoices, blockProbs, blockSwitch] = qLearningModel_absPePeLR_k_simNoPlot(varargin)
%
%
...
aF = a.Results.params(5);
aPE = a.Results.params(6);
beta = a.Results.params(7);
k = a.Results.params(8);
...
for currT = 1:p.MaxTrials - 1
    ...
    if binornd(1, pRight) == 0 % left choice selected probabilistically
        ...
        pePe(currT) = abs(abs(rpe) - peBar(currT));
        if rpe >= 0
            aP = pePe(currT) * aPscale + aPmin;
