function [t, allRewards, allChoices, blockProbs, blockSwitch, ITI] = qLearningModel_absPePe_scaleBoth_bias_simNoPlot(varargin)
%
%
...
for currT = 1:p.MaxTrials - 1
    ...
    if binornd(1, pRight) == 0 % left choice selected probabilistically
        ...
        if rpe(currT) >= 0
            Q(currT + 1, 1) = Q(currT, 1) + aP*rpe(currT) * (1 - peBar(currT + 1));
        else
        ...
        peBar(currT + 1) = peBar(currT) + aPE * pePe(currT);
        if rpe(currT) >= 0
            Q(currT + 1, 2) = Q(currT, 2) + aP*rpe(currT) * (1 - peBar(currT + 1));
        else
