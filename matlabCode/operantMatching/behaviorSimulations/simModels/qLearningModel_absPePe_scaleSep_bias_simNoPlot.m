function [t, allRewards, allChoices, blockProbs, blockSwitch, ITI] = qLearningModel_absPePe_scaleSep_bias_simNoPlot(varargin)
%
%
...
peBar = [0 0; NaN(a.Results.maxTrials-1, 2)];
...
for currT = 1:p.MaxTrials - 1
    ...
    if binornd(1, pRight) == 0 % left choice selected probabilistically
        ...
        pePe(currT) = abs(rpe(currT)) - peBar(currT, 1);
        peBar(currT + 1, 1) = peBar(currT, 1) + aPE * pePe(currT);
        if rpe(currT) >= 0
            Q(currT + 1, 1) = Q(currT, 1) + aP*rpe(currT);
        else
            Q(currT + 1, 1) = Q(currT, 1) + aN*rpe(currT) * (1 - peBar(currT + 1, 1));
        end
        Q(currT + 1, 2) = Q(currT, 2)*aF;
        peBar(currT + 1, 2) = peBar(currT, 2);
    else
        p.inputChoice([0 1]);
        allRewards(currT) = p.AllRewards(currT, 2);
        rpe(currT) = p.AllRewards(currT, 2) - Q(currT, 2);
        pePe(currT) = abs(rpe(currT)) - peBar(currT, 2);
        peBar(currT + 1, 2) = peBar(currT, 2) + aPE * pePe(currT);
        if rpe(currT) >= 0
            Q(currT + 1, 2) = Q(currT, 2) + aP*rpe(currT);
        else
            Q(currT + 1, 2) = Q(currT, 2) + aN*rpe(currT) * (1 - peBar(currT + 1, 2));
        end
        Q(currT + 1, 1) = Q(currT, 1)*aF;
        peBar(currT + 1, 1) = peBar(currT, 1);
    end
end
