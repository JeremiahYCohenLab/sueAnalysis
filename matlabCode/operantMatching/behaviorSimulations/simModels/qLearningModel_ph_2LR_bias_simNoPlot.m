function [t, allRewards, allChoices, blockProbs, blockSwitch] = qLearningModel_ph_2LR_bias_simNoPlot(varargin)
%
%
...
eta = a.Results.params(1);
kappaN = a.Results.params(2);
kappaP = a.Results.params(3);
...
alpha = zeros(1, a.Results.maxTrials+1);
...
for currT = 1:p.MaxTrials    
    ...
    if binornd(1, pRight) == 0 % left choice selected probabilistically
        ...
        if rpe > 0
            Q(currT+1, 1) = Q(currT, 1) + kappaN * alpha(currT) * rpe;
        else
            Q(currT+1, 1) = Q(currT, 1) + kappaP * alpha(currT) * rpe;
        end
        ...
        if rpe > 0
            Q(currT+1, 2) = Q(currT, 2) + kappaN * alpha(currT) * rpe;
        else
            Q(currT+1, 2) = Q(currT, 2) + kappaP * alpha(currT) * rpe;
        end
        Q(currT+1, 1) = Q(currT, 1)*aF;
    end
    alpha(currT+1) = eta * abs(rpe) + alpha(currT) * (1 - eta);
end
