function [t, allRewards, allChoices, blockProbs, blockSwitch] = qLearningModel_ph_bias_simNoPlot(varargin)
%
%
...
for currT = 1:p.MaxTrials - 1    
    ...
    if binornd(1, pRight) == 0 % left choice selected probabilistically
        ...
        Q(currT+1, 1) = Q(currT, 1) + kappa * alpha(currT+1) * rpe;
        ...
        alpha(currT+1) = eta * abs(rpe) + alpha(currT) * (1 - eta);
        Q(currT+1, 2) = Q(currT, 2) + kappa * alpha(currT+1) * rpe;
