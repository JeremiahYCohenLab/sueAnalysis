function [sessionData, blockSwitch, blockProbs] = generateSessionData_behav_operantMatching_RwdDelay(sessionName)
...
for i = 1:length(sessionText)
    ...
    if ~isempty(regexp(sessionText{i},'Trial ', 'once')) && isempty(find(regexp(sessionText{i},'Trial ') > 3, 1)) % trial begin 
        ...
        for currTrialInd = tBegin:tEnd
            ...
            if ~isempty(strfind(sessionText{currTrialInd},'LASER L DELIVERED')) || ~isempty(strfind(sessionText{currTrialInd},'LASER R DELIVERED'))
...
%             end
            ...
            if currTrialInd == tEnd % run this at the last index || currTrialInd == length(sessionText)-1
                ...
                if ~isempty([allL_licks allR_licks])
                    allLicks = [allL_licks allR_licks];
                    allLicks = allLicks(allLicks >= sessionData(currTrial).CSon);
                    sessionData(currTrial).respondTime = min(allLicks);
                else
