function [glm_rwdLick, rwdMatx, noRwdMatx, combinedPreLick, combinedLickLatZ] = combineLinRegLickLatTime_activation_opMD(xlFile, animal, category, varargin)
...
%     fprintf([sessionName '\n'])
    ...
    laserTmpMatx = zeros(tMax, length(responseInds));
    ...
    for j = 2:length(responseInds)          
        ...
        while j-k > 0 && behSessionData(responseInds(j)).respondTime - behSessionData(responseInds(j-k)).rewardTime < timeMax
            ...
            if behSessionData(responseInds(j-k)).laser == 1 
                timeTmpLaser = [timeTmpLaser (behSessionData(responseInds(j)).respondTime - behSessionData(responseInds(j-k)).rewardTime)];
            end
            k = k + 1;
        end
        ...
        if ~isempty(timeTmpLaser)
            binnedLaser = discretize(timeTmpLaser,timeBinEdges);
            for k = 1:tMax
                if ~isempty(find(binnedLaser == k, 1))
                    laserTmpMatx(k,j) = sum(binnedLaser == k);
                end
            end
    ...
    while behSessionData(responseInds(j)).respondTime - behSessionData(responseInds(1)).respondTime < timeMax
        ...
        laserTmpMatx(binnedDiff+1:tMax,j) = NaN;
        j = j+1;
    end
    ...
    laserTmpMatx(:,1) = NaN;
    laserMatx = [laserMatx NaN(length(timeBinEdges)-1, 100) laserTmpMatx];
    ...
    combinedLaser = [combinedLaser, NaN(1, 100), s.laser];
end

%linear regression model
glm_rwdLick = fitlm([rwdMatx'], combinedLickLatZ);
glm_rwdLickAll = fitlm([rwdMatx' noRwdMatx' laserMatx' combinedTimeInSesh' combinedChangeChoice' combinedPreLick'], combinedLickLatZ);
tbl = table(combinedPreLick', combinedChangeChoice', combinedLaser', rwdMatx(1,:)', noRwdMatx(1,:)', preITI',combinedLickLatZ',  'VariableNames', {'pre', 'sw', 'laser', 'rwd1', 'nRwd1', 'preITI', 'lickLat'});
...
if p.Results.plotFlag
    ...
    relevInds = 2*tMax+2:3*tMax+1;
    coefVals = glm_rwdLickAll.Coefficients.Estimate(relevInds);
    CIbands = coefCI(glm_rwdLickAll);
    errorL = abs(coefVals - CIbands(relevInds,1));
    errorU = abs(coefVals - CIbands(relevInds,2));
    errorbar(((1:tMax)*binSize/1000),coefVals,errorL,errorU,'Color', [1 0 0],'linewidth',2)
