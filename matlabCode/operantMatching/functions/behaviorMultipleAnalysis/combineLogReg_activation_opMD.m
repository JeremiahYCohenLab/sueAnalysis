function [glm_rwdNoRwd ,combinedRewardsMatx, combinedNoRewardsMatx, fitresult] = combineLogReg_activation_opMD(xlFile, animal, category, laserOutcome, varargin)
...
for i = 1: length(dayList)
    ...
    allActivation = [behSessionData(responseInds).laser];
    ...
    for j = 1:tMax
        ...
        activationMatxTmp(j,:) = [NaN(1,j) allActivation(1:end-j)];
    end
...
%    allRewards(allRewards == -1) = 1;
    ...
    combinedRewardsMatx = [combinedRewardsMatx rwdMatxTmp];
    combinedNoRewardsMatx = [combinedNoRewardsMatx noRwdMatxTmp];
    combinedChoicesMatx = [combinedChoicesMatx choiceMatxTmp];
...
if strcmp(laserOutcome, 'rwd')
    combinedLaseredOutcome = combinedRewardsMatx;
    combinedLaseredOutcome(combinedActivation == 0) = 0;
else
    if strcmp(laserOutcome, 'nRwd')
        combinedLaseredOutcome = combinedNoRewardsMatx;
        combinedLaseredOutcome(combinedActivation == 0) = 0;
    end
end
...
glm_rwdNoRwd = fitglm([combinedRewardsMatx' combinedNoRewardsMatx' combinedLaseredOutcome'], combinedAllChoice_R,'distribution','binomial','link','logit'); rsq{7} = num2str(round(glm_rwdNoRwd.Rsquared.Adjusted*100)/100);
