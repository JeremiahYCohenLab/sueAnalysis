function behAnalysis_opMD_RwdDelay_noblock(sessionName, varargin)
p = inputParser;
    ...
    if ~isempty(behSessionData(i).rewardTime)
        ...
        if ~isnan(behSessionData(i).rewardL)
            ...
            if behSessionData(i).rewardL == 1
                if length(find(behSessionData(i).licksL < behSessionData(i).rewardTime)) > 1
                    preLickTemp = behSessionData(i).licksL(behSessionData(i).licksL < behSessionData(i).rewardTime);
                    lickRateTemp = 1000/(min(diff(preLickTemp)));
        ...
        elseif ~isnan(behSessionData(i).rewardR)
            lickLat_R = [lickLat_R behSessionData(i).respondTime - behSessionData(i).CSon];      %make single licks zeros for easier indexing
            if behSessionData(i).rewardR == 1
                if length(find(behSessionData(i).licksR < behSessionData(i).rewardTime)) > 1
                    preLickTemp = behSessionData(i).licksR(behSessionData(i).licksR < behSessionData(i).rewardTime);
                    lickRateTemp = 1000/(min(diff(preLickTemp)));
                    lickRate = [lickRate lickRateTemp];
...
histogram(lickLat_L,0:25:1500,'Normalization','probability', 'FaceColor', 'm'); histogram(lickLat_R,0:25:1500,'Normalization','probability', 'FaceColor', 'c')
...
sessionInd = responseInds([s.dd] ~= 0);
doubleChoice = [s.allChoices([s.dd] ~= 0)];
doubleReward = [s.allRewards([s.dd] ~= 0)];

plot([sessionInd; sessionInd], [zeros(1, length(sessionInd)); 0.5 * (doubleChoice + doubleReward)], 'Color', 'b', 'LineWidth', 1.5);
