function compareLatVar_nSim_dF(xlFile, sheet, beh, varargin)
...
for currCell = 1:numCells
    ...
    spikesACF = autocorr(sessionSpikes, 30);
    acfSum(currCell) = sum(spikesACF);
    ...
    for currVar = 1:numVars
        ...
        for currS = 1:numSims
            spikeSim = [];
            spikeSim(1:intL) = 0; 
            for currT = intL+1:length(sessionSpikes)+intL
                spikeSim(currT) = sum(spikeSim(currT-intL:currT-1).*intKern) + normrnd(0,1);
            end
            
            mdl = fitlm(zscore(sessionVar)', spikeSim(intL+1:end)');
            simT(currCell, currVar, currS) = mdl.Coefficients.tStat(2);
            
            simACF = autocorr(spikeSim(6:end), 30);
            diffTmp(currS) = sum(abs([spikesACF - simACF]));
            sumTmp(currS) = sum(simACF);
        end
