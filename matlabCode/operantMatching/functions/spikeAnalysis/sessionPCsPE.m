function sessionPCsPE(session, col, varargin)
...
combineMat = [pe, outcome', rightSide',os.lickInds,svs'];
regressors = {'pe', 'outcome', 'rightSide','lick','svs'};
...
for i = 1:length(clust)
   ...
   for j = 1:length(allTrial_spike_choice)
    ...
    allTrial_spikeMatx_slide = reshape(allTrial_spikeMatx_slide', [1,  numel(allTrial_spikeMatx_slide)]);

    allSpikes = [allSpikes, allTrial_spikeMatx_slide'];
...
%     end
    ...
    [PCs,score,latent] = pca(allSpikes);

for i = 1:length(clust) %first three PCs
    coeff = zeros(length(midPoints),size(combineMat,2)+1,3);
    rsq = zeros(size(midPoints));
    sigs = zeros(length(midPoints),size(combineMat,2)+1);
    allTrial_spikeMatx_slide = reshape(score(:,i), [length(midPoints), length(os.responseInds)])';
...
figure;
subplot(1,2,1);
varExp = zeros(size(latent));
for t = 1:length(latent)
    varExp(t) = sum(latent(1:t))/sum(latent);
end
plot(varExp);
ylim([0 1]);
title('varExplained');
