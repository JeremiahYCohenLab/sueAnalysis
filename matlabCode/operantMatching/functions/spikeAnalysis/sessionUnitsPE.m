function sessionUnitsPE(session, col, varargin)
...
%     line([startTimeLate-1000*p.Results.tb endTimeLate-1000*p.Results.tb],[max(coeff(:,2:end,3),[],'all'),max(coeff(:,2:end,3),[],'all')],'Color', [0 0 1], 'LineWidth',4)
    ...
    title(sessionUnits{i}, 'interpreter', 'none');
    ...
    legend({'L','R'})
    title('Spike rate change with left/right pe')
    ...
    subplot(3,length(clust)+1,1); hold on;
    title('Spike rate change with pe')  
    legend(sessionUnits, 'Location', 'best', 'interpreter', 'none');

    subplot(3,length(clust)+1,1+(length(clust)+1)); hold on;
    title(['Spike rate change with left pe' num2str(length(os.lickL_Inds))])
    
    subplot(3,length(clust)+1,1+2*(length(clust)+1)); hold on;
    title(['Spike rate change with right pe' num2str(length(os.lickR_Inds))])
