function spikeCatbyOutcome(animalNames, category, varargin)
...
%     eval(['targetVar =' p.Results.catRegressor ';'])
    ...
    %% calculate and combine SW in nrwd trials
    % bin trials into spike number bins and calcualted mean of bins
    numBins = 4;
    currTarget = spikeCounts(os.nrwd_Inds);
    subject = tbl.svsNext(os.nrwd_Inds);
    subject2 = tbl.choiceKernel(os.nrwd_Inds);
    edges = linspace(min(currTarget)-0.001, max(currTarget)+0.001, numBins+1);
    subjectMeans = zeros(1,numBins);
    subjectMeans2 = zeros(1,numBins);
    targetMeans = zeros(1,numBins);
    ...
    % collecting neuron data based on significance
    subjectMeans(~isnan(subjectMeans)) = zscore(subjectMeans(~isnan(subjectMeans)));
    allSWNrwd = [allSWNrwd; subjectMeans];
    subjectMeans2(~isnan(subjectMeans2)) = zscore(subjectMeans2(~isnan(subjectMeans2)));
    allKernelNrwd = [allKernelNrwd; subjectMeans2];
    focusSpikesNrwdBins = [focusSpikesNrwdBins; targetMeans];
%%  calculate and combine SW in rwd trials
    % bin trials into spike number bins and calcualted mean of bins
    numBins = 4;
    currTarget = spikeCounts(os.rwd_Inds);
    subject = tbl.svsNext(os.rwd_Inds);
    subject2 = tbl.choiceKernel(os.rwd_Inds);
    edges = linspace(min(currTarget)-0.001, max(currTarget)+0.001, numBins+1);
    subjectMeans = zeros(1,numBins);
    subjectMeans2 = zeros(1,numBins);
    targetMeans = zeros(1,numBins);
    ...
    % collecting neuron data based on significance
    subjectMeans(~isnan(subjectMeans)) = zscore(subjectMeans(~isnan(subjectMeans)));
    allSWRwd = [allSWRwd; subjectMeans];
    subjectMeans2(~isnan(subjectMeans2)) = zscore(subjectMeans2(~isnan(subjectMeans2)));
    allKernelRwd = [allKernelRwd; subjectMeans2];
    focusSpikesRwdBins = [focusSpikesRwdBins; targetMeans];
    %%
...
%             spikeGLM_dF(session, 'good','cellName', unit,'regressors','1  + Qchosen + outcome*rightSide');
    ...
    allCoeffs(length(allSessions),:) = lm.Coefficients.Estimate(2:end);
    allTstats(length(allSessions),:) = lm.Coefficients.tStat(2:end);
    allSigs(length(allSessions),:) = lm.Coefficients.pValue(2:end);
    regressors = lm.CoefficientNames(2:end);
    
    lmPe = fitlm(currTbl, ['spikeCounts~ 1 + pe + rightSide']);
    allCoeffsPe(length(allSessions),:) = lmPe.Coefficients.Estimate(2:end);
    allTstatsPe(length(allSessions),:) = lmPe.Coefficients.tStat(2:end);
    allSigsPe(length(allSessions),:) = lmPe.Coefficients.pValue(2:end);
    regressorsPe = lmPe.CoefficientNames(2:end);
