function spikeGLMGridSide_populationPE(animalNames, category, varargin)
...
p.addParameter('yRegressor', 'Qchosen')
p.addParameter('plotRegressor', 'pe')
p.addParameter('binSize', 100)% in ms
p.addParameter('stepSize', 100)
p.addParameter('tb', 1)% in sm 
p.addParameter('tf', 3)% in s
p.addParameter('focusWin', [300 1300]); % in ms from respond time
...
%% session and unit loop
    ...
    if ~strcmp(session,prevSession)
        [~,~,noSession] = getStanModelParams_samps(p.Results.modelName, [path sampFile '.mat'], 1, 'sessionName', session);
        prevSession = session;
    end
...
ind = kmeans(scoreAll(:, 1:5), 2);
% plot
figure2;
subplot(1, size(matrix,2)+2, 1); hold on;
scatter3(scoreAll(ind==1,1), scoreAll(ind==1,2), scoreAll(ind==1,3));
scatter3(scoreAll(ind==2,1), scoreAll(ind==2,2), scoreAll(ind==2,3));
legend('1','2')
subplot(1,size(matrix,2)+2, 2); hold on;
mat = waveforms(ind==1,:)+m;
plotFilled(1:length(m), mat, 'b');
mat = waveforms(ind==2,:)+m;
plotFilled(1:length(m), mat, 'r');
for i = 1:size(matrix,2)
    edges = linspace(min(matrix(:,i))-0.001, max(matrix(:,i)+0.001),10);
    subplot(1, size(matrix,2)+2, i+2);
    hold on;
    histogram(matrix(ind==1,i), edges);
    histogram(matrix(ind==2,i), edges);
    title(names{i})
end
%% animal loop 
for ani = 1:length(animalNames)
...
%% session and unit loop
    ...
    if ~strcmp(session,prevSession) % avoiding recomputing model variables for different unit from same session
        ...
        for cols = 1:length(names)
            tmp = tbl.(names{cols});
            if sum(ismember(tmp(~isnan(tmp)), [0 1 -1])) < sum(~isnan(tmp))
    ...
    % decide if group is right
    if ~ismember(group, ind(neuronNum))
        continue
    end
...
%         fprintf([num2str(w), '\n'])
   ...
   for j = 1:length(allTrial_spike_choice)
    ...
    numBins = 10;
    currTarget = tbl.(p.Results.plotRegressor);
    ...
    allPe{2+sigs(yRegressorInd),2+sigs(xRegressorInd)} = [allPe{2+sigs(yRegressorInd),2+sigs(xRegressorInd)}; targetMeans];
    focusSpikes{2+sigs(yRegressorInd),2+sigs(xRegressorInd)} = [focusSpikes{2+sigs(yRegressorInd),2+sigs(xRegressorInd)}; spikeMeans];
