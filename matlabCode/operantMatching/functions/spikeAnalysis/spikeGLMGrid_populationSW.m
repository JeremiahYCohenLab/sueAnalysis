function spikeGLMGrid_populationSW(animalNames, category, varargin)
...
p.addParameter('plotRegressor', 'svsNext')
...
%% session and unit loop
    ...
    if ~strcmp(session,prevSession) % avoiding recomputing model variables for different unit from same session
        ...
        if contains(p.Results.modelName, '7params_absPePeAN_scale_int_bias_ord')
            ...
            pePe = t.pePe;
            scPe = pe.*(1-peBar);
            tbl = table(outcome, pe, prePe, preRwd, Qsum, Qdiff, choiceConf, biasSide, rightSide, timeInSession, lickLat, hmm, Qchosen, Qunchosen, QchosenUpdate, svs, svsNext, preITI, dawExp, scPe, aN, peBar, pePe);
        else
            tbl = table(outcome, pe, prePe, preRwd, Qsum, Qdiff, choiceConf, biasSide, rightSide, timeInSession, lickLat, hmm, Qchosen, Qunchosen, QchosenUpdate, svs, svsNext, preITI, dawExp);
        end
...
%         fprintf([num2str(w), '\n'])
   ...
   for j = 1:length(allTrial_spike_choice)
    ...
    subject = tbl.(p.Results.plotRegressor);
    ...
    focusSpikes{2+sigs(yRegressorInd),2+sigs(xRegressorInd)} = [focusSpikes{2+sigs(yRegressorInd),2+sigs(xRegressorInd)}; subjectMeans];
