    function spikeGLMPosition_population(animalNames, category, varargin)
...
p.addParameter('regressors', '1+pe+outcome*rightSide')
p.addParameter('plotRegressor', 'pe')
p.addParameter('focusWin', [300 1000]); % in ms from respond
...
%         fprintf([num2str(w), '\n'])
   ...
   for j = 1:length(allTrial_spike_choice)
    ...
    populationSig = [populationSig; sigTmp];
    populationTStats  = [populationTStats; tStatsTmp];
    populationCoeffs  = [populationCoeffs; coeffsTmp];
    end 
end
...
suptitle('position distribution')
edges = linspace(min(populationDepths)-0.001, max(populationDepths)+0.001, 20);
binMeans = (edges(1:end-1)+edges(2:end))/2; 
%% count number of units in different bins


% 
for i = 1:length(regressors)
%     [~,seq] = sort(populationTStats(:,i));
    posInd = populationSig(:,i)>0;
    negInd = populationSig(:,i)<0;
    nonInd = populationSig(:,i)==0;
    
    allDist = histcounts(populationDepths, edges);
    allPos = histcounts(populationDepths(posInd), edges)./allDist;
    allPos(isnan(allPos)) = 0;
    
    allNeg = histcounts(populationDepths(negInd), edges)./allDist;
    allNeg(isnan(allNeg)) = 0;
    
    subplot(length(regressors),2,2*(i-1)+1); hold on;
    plot(binMeans, allPos, 'Color', 'm', 'linewidth', 2);
    plot(binMeans, allNeg, 'Color', 'c', 'linewidth', 2);
    legend('+','-')
    title([regressors{i} ' pos ' num2str(sum(posInd)) 'neg' num2str(sum(negInd))]);
    subplot(length(regressors),2,2*(i-1)+2); hold on;
    histogram(populationDepths, edges, 'FaceColor', [0.7 0.7 0.7], 'Normalization', 'probability');
    title([regressors{i} ' all ']);
end
