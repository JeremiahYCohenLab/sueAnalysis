    function spikeGLM_dF_allRegressors(session, col, varargin)
...
p.addParameter('regressors', '1+pe*biasSide')
...
if contains(p.Results.modelName, '7params_absPePeAN_scale_int_bias_ord')
    ...
    pePe = t.pePe;
    scPe = pe.*(1-peBar);
    tbl = table(outcome, pe, prePe, preRwd, Qsum, Qdiff, choiceConf, biasSide, rightSide, timeInSession, lickLat, hmm, Qchosen, dawExp, scPe, aN, peBar, pePe);
else
    tbl = table(outcome, pe, prePe, preRwd, Qsum, Qdiff, choiceConf, biasSide, rightSide, timeInSession, lickLat, hmm, Qchosen, dawExp);
end
...
%     end
    ...
    for r = 1:length(regressors)
        eval(['tmp = ' regressors{r} ';']);
        if length(unique(tmp))>10 && ~contains(regressors{r},':')
            plotRegressors = [plotRegressors, regressors{r}];
        end
    end
    ...
    colorR = [1 0 1];
    colorL = [0 1 1];
    for j = 1:length(plotRegressors)
        eval(['target =' plotRegressors{j} ';']);   
        % decide start of the time windows
        startTime = 1000*p.Results.tb + p.Results.focusWin(1);
        endTime  =1000*p.Results.tb + p.Results.focusWin(2);  
        spikeCounts = nansum(allTrial_spikeMatx_choice(:,startTime:endTime),2)*1000/(endTime - startTime);
        ...
        subplot(3,length(plotRegressors),length(plotRegressors)+j); hold on;
        errorbar(targetMeans, spikeMeans, spikeSems,'linewidth', 2)
        title(['Spike rate change with ' plotRegressors{j}])
    
        subplot(3,length(plotRegressors),2*length(plotRegressors)+j); hold on;
        ...
        title(sprintf('L/R = %d/%d', length(os.lickL_Inds), length(os.lickR_Inds)))
    end
    %% plot glm
