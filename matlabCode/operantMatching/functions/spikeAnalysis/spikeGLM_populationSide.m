...
p.parse(varargin{:});
populationSigL = []; % the matrix with 1 for positive beta, -1 for negative beta
populationSigR = [];
populationTStatsL = []; % t statistics for each parameter
populationTStatsR = [];
populationCoeffsL =[]; % coeffs for each regressor
populationCoeffsR =[];
...
%         fprintf([num2str(w), '\n'])
   ...
   for j = 1:length(allTrial_spike_choice)
    ...
    for k = 1:length(midPoints)
        ...
        currSpikesL = currSpikes(choice==0);
        currSpikesR = currSpikes(choice==1);
        tblL = tbl(choice==0,:);
        tblR = tbl(choice==1,:);
        currTblL = addvars(tblL, currSpikesL);
        currTblR = addvars(tblR, currSpikesR);
        lmL = fitlm(currTblL, ['currSpikesL~' p.Results.regressors]);
        lmR = fitlm(currTblR, ['currSpikesR~' p.Results.regressors]);
        
        sigTmpL = zeros(1,length(lmL.CoefficientNames)-1);
        tStatsTmpL = zeros(1,length(lmL.CoefficientNames)-1);
        coeffsTmpL = zeros(1,length(lmL.CoefficientNames)-1);
        sigTmpR = zeros(1,length(lmR.CoefficientNames)-1);
        tStatsTmpR = zeros(1,length(lmR.CoefficientNames)-1);
        coeffsTmpR = zeros(1,length(lmR.CoefficientNames)-1);
        for j = 1:(length(lmL.CoefficientNames)-1)
            if lmL.Coefficients.pValue(j+1)<0.05
               sigTmpL(j) = sign(lmL.Coefficients.Estimate(j+1));
            else
               sigTmpL(j) = 0;
            end
            tStatsTmpL(j) = lmL.Coefficients.tStat(j+1);
            coeffsTmpL(j) = lmL.Coefficients.Estimate(j+1);
        end
        sigsL = [sigsL; sigTmpL];
        tStatsL = [tStatsL; tStatsTmpL];
        coeffsL = [coeffsL; coeffsTmpL];
        
        for j = 1:(length(lmR.CoefficientNames)-1)
            if lmR.Coefficients.pValue(j+1)<0.05
               sigTmpR(j) = sign(lmR.Coefficients.Estimate(j+1));
            else
               sigTmpR(j) = 0;
            end
            tStatsTmpR(j) = lmR.Coefficients.tStat(j+1);
            coeffsTmpR(j) = lmR.Coefficients.Estimate(j+1);
        end
        sigsR = [sigsR; sigTmpR];
        tStatsR = [tStatsR; tStatsTmpR];
        coeffsR = [coeffsR; coeffsTmpR];
    ...
    populationSigL = cat(3, populationSigL, sigsL);
    populationSigR = cat(3, populationSigR, sigsR);
    populationTStatsL  = cat(3,populationTStatsL, tStatsL);
    populationTStatsR  = cat(3,populationTStatsR, tStatsR);
    populationCoeffsL  = cat(3,populationCoeffsL, coeffsL);
    populationCoeffsR  = cat(3,populationCoeffsR, coeffsR);
    end 
end

%% plot everything
regressors = lmL.CoefficientNames(2:end);
tFig = figure;
