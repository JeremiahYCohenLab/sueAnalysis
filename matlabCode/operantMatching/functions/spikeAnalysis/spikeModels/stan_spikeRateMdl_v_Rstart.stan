...
transformed parameters{
 ...
 vector<lower=0, upper=1>[N] Rstart;
  ...
  for (n in 1:N) {
    ...
    Rstart[n] = Phi_approx(Rstart_pr[n]);
  }
}
model {
  v_pr      ~ normal(0,1);
  ...
  for (n in 1:N) {
    ...
    R[1] = Rstart[n];                 // parameterized start value

    for (t in 1:(Tsesh[n] - 1)) {
      R[t+1] = v[n] * outcome[n, t] + (1-v[n]) * R[t];
    }
    spikes[n, 1:Tsesh[n]] ~ normal(slope[n]*R + intercept[n], sigma[n]); 
  }
}
...
generated quantities{
  ...
  for (n in 1:N){
    R[n,1] = Rstart[n];               // parameterized start value

    for (t in 1:(Tsesh[n] - 1)) {
      R[n, t+1] = v[n] * outcome[n, t] + (1-v[n]) * R[n,t];
    }

    spikes_pred[n, 1:Tsesh[n]] = slope[n]*R[n,1:Tsesh[n]] + intercept[n];       //might want to convert this to median and std before matlab to save memory/time
  }
}
