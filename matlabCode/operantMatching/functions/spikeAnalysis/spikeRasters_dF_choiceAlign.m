...
for k = 1:length(s.responseInds)
    for i = 1:length(clust)
        if s.responseInds(k) == 1
            ...
            prevTrial_spikeInd = [sessionData(s.responseInds(k)-1).(spikeFields{clust(i)})] > (sessionData(s.responseInds(k)).respondTime-p.Results.tb*1000);
            prevTrial_spike = sessionData(s.responseInds(k)-1).(spikeFields{clust(i)})(prevTrial_spikeInd) - sessionData(s.responseInds(k)).respondTime;
        end
        
        currTrial_spikeInd = sessionData(s.responseInds(k)).(spikeFields{clust(i)}) < sessionData(s.responseInds(k)).respondTime+p.Results.tf*1000 ... 
            & sessionData(s.responseInds(k)).(spikeFields{clust(i)}) > sessionData(s.responseInds(k)).respondTime-p.Results.tb*1000;
        currTrial_spike = sessionData(s.responseInds(k)).(spikeFields{clust(i)})(currTrial_spikeInd) - sessionData(s.responseInds(k)).respondTime;
...
%     ylim([0 maxLick]);
    ...
    mySDF_winStay_spike = allTrial_spikeMatx_slide(intersect(s.rwd_Inds, s.stayChoice_Inds(2:end)-1),:);
    mySDF_winShift_spike = allTrial_spikeMatx_slide(intersect(s.rwd_Inds, s.changeChoice_Inds-1),:);
    plotFilled(slideTime, mySDF_winStay_spike,[0 0 1])
    if size(mySDF_winShift_spike,1)>1
        plotFilled(slideTime, mySDF_winShift_spike,[0.7 0 1])
        line([0 0], [0 maxLick], 'color', 'r')
        legend('winStay', '', 'winShift', '')
    else
        if size(mySDF_winShift_spike,1)==1
            plot(slideTime, mySDF_winShift_spike,'color',[0.7 0 1])
            line([0 0], [0 maxLick], 'color', 'r')
            legend('winStay', '', 'winShift')
        end
    end
...
%     
    ...
    if downsampFlag == true
        text( 0.5, 0, [sessionName ': ' spikeFields{clust(i)} ' Aligned to choice' '. Downsampled By ' num2str(downsampFactor)], 'FontSize', 14', 'FontWeight', 'Bold', ...
          'HorizontalAlignment', 'Center', 'VerticalAlignment', 'Bottom','Interpreter','none') ;
    else
        text( 0.5, 0, [sessionName ': ' spikeFields{clust(i)} ' Aligned to choice'], 'FontSize', 14', 'FontWeight', 'Bold', ...
          'HorizontalAlignment', 'Center', 'VerticalAlignment', 'Bottom','Interpreter','none') ;
    end
    
    
    if p.Results.saveFigFlag == 1 
        saveFigurePDF(rasters,[savepath sep sessionName '_' spikeFields{clust(i)} 'choiceAlinged'])
    end
end