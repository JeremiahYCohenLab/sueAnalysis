  function spikeRasters_dF_rwdAlign(sessionName, varargin)
...
%% find all aligning time points
timeAlign = zeros(size(s.responseInds));
% rwd trials using rwd delivery time
timeAlign(abs(s.allRewards)>0) = [sessionData(s.responseInds(abs(s.allRewards)>0)).rewardTime];
% non-rwd trials using first lick after rwd
for k = 1:length(s.responseInds)
    if abs(s.allRewards(k))>0
        timeAlign(k) = sessionData(s.responseInds(k)).rewardTime;
    else
        if s.allChoices(k)>0
            licksTemp = sessionData(s.responseInds(k)).licksR;
        else
            licksTemp = sessionData(s.responseInds(k)).licksL;
        end
        licksTemp = licksTemp(licksTemp>=sessionData(s.responseInds(k)).rewardTime);
        if isempty(licksTemp) || (min(licksTemp) - sessionData(s.responseInds(k)).rewardTime) > 500
            timeAlign(k) = sessionData(s.responseInds(k)).rewardTime;
        else
            timeAlign(k) = min(licksTemp);
        end
    end
end
...
for k = 1:length(s.responseInds)
    for i = 1:length(clust)
        if s.responseInds(k) == 1
            prevTrial_spike = [];
        else
            prevTrial_spikeInd = [sessionData(s.responseInds(k)-1).(spikeFields{clust(i)})] > (timeAlign(k)- p.Results.tb*1000);
            prevTrial_spike = sessionData(s.responseInds(k)-1).(spikeFields{clust(i)})(prevTrial_spikeInd) - timeAlign(k);
        end
        
        currTrial_spikeInd = sessionData(s.responseInds(k)).(spikeFields{clust(i)}) < timeAlign(k)+p.Results.tf*1000 ... 
            & sessionData(s.responseInds(k)).(spikeFields{clust(i)}) > timeAlign(k)-p.Results.tb*1000;
        currTrial_spike = sessionData(s.responseInds(k)).(spikeFields{clust(i)})(currTrial_spikeInd) - timeAlign(k);
    ...
    if ~isnan(sessionData(s.responseInds(k)).rewardL)
        currTrial_lickInd = [sessionData(s.responseInds(k)).licksL] < timeAlign(k) + p.Results.tf*1000;
        currTrial_lick = sessionData(s.responseInds(k)).licksL(currTrial_lickInd) - timeAlign(k);
    elseif ~isnan(sessionData(s.responseInds(k)).rewardR)
        currTrial_lickInd = [sessionData(s.responseInds(k)).licksR] < timeAlign(k) + p.Results.tf*1000;
        currTrial_lick = sessionData(s.responseInds(k)).licksR(currTrial_lickInd) - timeAlign(k);  
    else
        currTrial_lick = 0;
    end
...
for i = 1:length(clust)
    ...
    allTrial_spikeMatx_choice = zeros(length(s.responseInds),length(time));         
   ...
   for j = 1:length(allTrial_spike_choice)
    ...
    % slide window
    allTrial_spikeMatx_slide = zeros(length(s.responseInds), length(midPoints));
    for w = 1:length(midPoints)
    ...
    rwd_spikeMatx = allTrial_spikeMatx_slide(s.rwd_Inds,:);
    noRwd_spikeMatx = allTrial_spikeMatx_slide(s.nrwd_Inds,:);
    rwdPrev_spikeMatx = allTrial_spikeMatx_slide(s.rwd_Inds(1:end-1)+1,:);
    noRwdPrev_spikeMatx = allTrial_spikeMatx_slide(s.nrwd_Inds(1:end-1)+1,:);
    changeChoice_spikeMatx = allTrial_spikeMatx_slide(s.changeChoice_Inds,:);
    stayChoice_spikeMatx = allTrial_spikeMatx_slide(s.stayChoice_Inds,:);
  
   
    %sort spikes by reward history
    rwdHxI_spikeMatx = allTrial_spikeMatx_slide(rwdHxI_Inds,:);
    rwdHxII_spikeMatx = allTrial_spikeMatx_slide(rwdHxII_Inds,:);
    rwdHxIII_spikeMatx = allTrial_spikeMatx_slide(rwdHxIII_Inds,:);
    ...
    maxFreq = 1.2*max(mean(allTrial_spikeMatx_slide), [], 'all');
    maxLick = 1.2*max(mean(allTrial_lickMatx_slide), [], 'all');
    ...
    if ~all(cellfun(@isempty,allTrial_spike_choice(i,blockR)))
        plotSpikeRaster(allTrial_spike_choice(i,blockR),'PlotType','vertline'); hold on
    ...
    if ~all(cellfun(@isempty,allTrial_spike_choice(i,blockL)))
        plotSpikeRaster(allTrial_spike_choice(i,blockL),'PlotType','vertline'); hold on
    ...
    if ~all(cellfun(@isempty,allTrial_spike_choice(i,s.lickR_Inds)))
        plotSpikeRaster(allTrial_spike_choice(i,s.lickR_Inds),'PlotType','vertline'); hold on
    ...
    if ~all(cellfun(@isempty,allTrial_spike_choice(i,s.lickL_Inds)))
        plotSpikeRaster(allTrial_spike_choice(i,s.lickL_Inds),'PlotType','vertline'); hold on  
    ...
    if ~all(cellfun(@isempty,allTrial_spike_choice(i,s.changeChoice_Inds)))
        plotSpikeRaster(allTrial_spike_choice(i,[s.changeChoice_Inds s.stayChoice_Inds]),'PlotType','vertline'); hold on
    ...
    if ~all(cellfun(@isempty,allTrial_spike_choice(i,s.hmmStates==1)))
        plotSpikeRaster(allTrial_spike_choice(i,[find(s.hmmStates==1) find(s.hmmStates~=1)]),'PlotType','vertline'); hold on
    ...
    rwdHxOutcome = [allTrial_spike_choice(i,rwdHx_Inds(rwdHxNoRwd_Inds)) allTrial_spike_choice(i,rwdHx_Inds(rwdHxRwd_Inds))];
...
%    plot(time, mySDF_rwdHxIVnoRwd(1:length(time)),'g','LineWidth',2)
    ...
    mySDF_stayRwd = allTrial_spikeMatx_slide(intersect(s.rwd_Inds, s.stayChoice_Inds),:);
    mySDF_switchRwd = allTrial_spikeMatx_slide(intersect(s.rwd_Inds, s.changeChoice_Inds),:);
    ...
    mySDF_stayNoRwd = allTrial_spikeMatx_slide(intersect(s.nrwd_Inds, s.stayChoice_Inds),:);
    mySDF_switchNoRwd = allTrial_spikeMatx_slide(intersect(s.nrwd_Inds, s.changeChoice_Inds),:);
    ...
    mySDF_switchRwd = allTrial_spikeMatx_slide(intersect(s.rwd_Inds + 1, s.changeChoice_Inds),:);
    mySDF_switchNoRwd = allTrial_spikeMatx_slide(intersect(s.nrwd_Inds + 1, s.changeChoice_Inds),:);
    ...
    mySDF_stayRwd = allTrial_spikeMatx_slide(intersect(s.rwd_Inds + 1, s.stayChoice_Inds),:);
    mySDF_stayNoRwd = allTrial_spikeMatx_slide(intersect(s.nrwd_Inds + 1, s.stayChoice_Inds),:);
    ...
    mySDF_ore = allTrial_spikeMatx_slide(s.hmmStates==1,:);
    mySDF_oit = allTrial_spikeMatx_slide(s.hmmStates~=1,:);
    ...
    mySDF_exploreRwd = allTrial_spikeMatx_slide(intersect(s.rwd_Inds, find(s.hmmStates==1)),:);
    mySDF_exploreNoRwd = allTrial_spikeMatx_slide(intersect(s.nrwd_Inds, find(s.hmmStates==1)),:);
    ...
    mySDF_exploitRwd = allTrial_spikeMatx_slide(intersect(s.rwd_Inds, find(s.hmmStates~=1)),:);
    mySDF_exploitNoRwd = allTrial_spikeMatx_slide(intersect(s.nrwd_Inds, find(s.hmmStates~=1)),:);
...
%     
    ...
    if downsampFlag == true
        text( 0.5, 0, [sessionName ': ' spikeFields{clust(i)} ' Aligned to reward' '. Downsampled By ' num2str(downsampFactor)], 'FontSize', 14', 'FontWeight', 'Bold', ...
          'HorizontalAlignment', 'Center', 'VerticalAlignment', 'Bottom','Interpreter','none') ;
    else
        text( 0.5, 0, [sessionName ': ' spikeFields{clust(i)} ' Aligned to reward'], 'FontSize', 14', 'FontWeight', 'Bold', ...
          'HorizontalAlignment', 'Center', 'VerticalAlignment', 'Bottom','Interpreter','none') ;
    end
    
    
    if p.Results.saveFigFlag == 1 
        saveFigurePDF(rasters,[savepath sep sessionName '_' spikeFields{clust(i)} 'rwdAlinged'])
    end
end