...
% Initialize models
modelNames = {'fiveParams_bilal'};
startValueCSVs = {'qLearningModel_5params_bilal_startValues.csv'};
...
for currMod = 1:length(modelNames)
    ...
    elseif strcmp(modelNames{currMod}, 'fiveParams_bilal')
        b=[ alphaLearn_range(2);  alphaForget_range(2);  beta_range(2); kappa_range(2);  bias_range(2);
           -alphaLearn_range(1); -alphaForget_range(1); -beta_range(1); -kappa_range(1); -bias_range(1)];
        for r = 1:runs
            [allParams(r, :), LH(r, :), exitFl(r, :), ~, ~, ~, hess(r, :, :)] = ...
                fmincon(@qLearningModel_5params_bilal, startValues(r,:), A, b, [], [], [], [], [], options, choice, outcome);
        end
        [~,bestFit] = min(LH);
        model.(modelNames{currMod}).bestParams = allParams(bestFit, :);
        [~, model.(modelNames{currMod}).probChoice, model.(modelNames{currMod}).Q, model.(modelNames{currMod}).pe] = qLearningModel_5params_bilal(model.(modelNames{currMod}).bestParams, choice, outcome);
    else
