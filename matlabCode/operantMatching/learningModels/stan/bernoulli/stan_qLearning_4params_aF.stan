...
transformed data {
  ...
  real<lower=-100, upper=100> mu_paP;
  real<lower=-100, upper=100> mu_paN;
  real<lower=-100, upper=100> mu_pbeta;


  initQ = rep_vector(0.0, 2);
  mu_paP = inv_Phi(mu_aP);
  mu_paN = inv_Phi(mu_aN);
  mu_pbeta = inv_Phi(0.1 * mu_beta);
...
// Transform session-level raw parameters
  ...
  for (n in 1:N) {
    aN[n]   = Phi_approx(mu_paN + sigma[1] * aN_pr[n]);
    aP[n]   = Phi_approx(mu_paP + sigma[2] * aP_pr[n]);
    aF[n]   = Phi_approx(mu_p + sigma[3] * aF_pr[n]);
    beta[n] = Phi_approx(mu_pbeta + sigma[4] * beta_pr[n]) * 10;
  }
}
model {
  // Hyperparameters
