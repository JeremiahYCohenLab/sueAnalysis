...
transformed data {
  ...
  real<lower=-100, upper=100> mu_paF;


  initQ = rep_vector(0.0, 2);
  mu_paN = inv_Phi(mu_aN);
  mu_paP = inv_Phi(mu_aP);
  mu_paF = inv_Phi(mu_aF);
...
// Transform session-level raw parameters
  ...
  for (n in 1:N) {
    ...
    aF[n]   = Phi_approx(mu_paF + sigma[3] * aF_pr[n]);
    beta[n] = Phi_approx(mu_p + sigma[4] * beta_pr[n]) * 10;
  }
}
model {
  // Hyperparameters
