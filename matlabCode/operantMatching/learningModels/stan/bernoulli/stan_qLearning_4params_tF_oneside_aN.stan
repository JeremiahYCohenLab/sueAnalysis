...
// Declare all parameters as vectors for vectorizing
  ...
  vector[N] aN_pr;    // learning rate for PPE
}
...
// Transform session-level raw parameters
  ...
  for (n in 1:N) {
    aN[n]   = Phi_approx(mu_p + sigma * aN_pr[n]);
  }
}
model {
  // Hyperparameters
