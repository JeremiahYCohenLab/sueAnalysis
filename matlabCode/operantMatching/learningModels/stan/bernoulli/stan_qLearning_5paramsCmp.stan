data {
  int<lower=1> N;
  int<lower=1> N1; //number of sessions in group1
  int<lower=1> T;
...
// Declare all parameters as vectors for vectorizing
  ...
  vector[N1] diff_pr; // parameter difference
}
...
// Transform session-level raw parameters
  ...
  vector[N1] diff;
  for (n in 1:N1) {
      diff[n] = mu_p[5] + sigmaDiff * diff_pr[n];
  }
  ...
  for (n in 1:N) {
    ...
    if (n <= N1){
      if (cmpParam == 1){
        aN[n] = Phi_approx(mu_p[1] + sigma[1] * aN_pr[n] + diff[n]);
      }else if (cmpParam == 2){
        aP[n] = Phi_approx(mu_p[2] + sigma[2] * aP_pr[n] + diff[n]);
      }else if (cmpParam == 3){
        aF[n] = Phi_approx(mu_p[3] + sigma[3] * aF_pr[n] + diff[n]);
      }else{
        beta[n] = Phi_approx(mu_p[4] + sigma[4] * beta_pr[n] + diff[n])*10;
      }
    }
  }
}
...
generated quantities {
  ...
  if (cmpParam == 1) {
    mu_aN_diff = Phi_approx(mu_p[1] + mu_p[5]);
  }else if (cmpParam == 2){
    mu_aP_diff = Phi_approx(mu_p[2] + mu_p[5]);
  }else if (cmpParam ==3) {
    mu_aF_diff = Phi_approx(mu_p[3] + mu_p[5]);
  }else{
    mu_beta_diff = Phi_approx(mu_p[4] + mu_p[5])*10;
  }
