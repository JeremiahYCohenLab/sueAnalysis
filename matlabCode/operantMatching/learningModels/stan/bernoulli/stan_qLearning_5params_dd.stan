...
// Declare all parameters as vectors for vectorizing
  ...
  vector[N] aDD_pr;   // learning rate for double dipping
...
// Transform session-level raw parameters
  ...
  vector<lower=-0.5, upper=0.5>[N] aDD;
  ...
  for (n in 1:N) {
    ...
    aDD[n] = Phi_approx(mu_p[4] + sigma[4] * aDD_pr[n]) - 0.5;
...
generated quantities {
  ...
  real<lower=-0.5, upper=0.5> mu_aDD;
