...
// Transform session-level raw parameters
  ...
  for (n in 1:N) {
    aN[n]   = inv_logit(mu_p[1] + sigma[1] * aN_pr[n]);
    aP[n]   = inv_logit(mu_p[2] + sigma[2] * aP_pr[n]);
    aF[n]   = inv_logit(mu_p[3] + sigma[3] * aF_pr[n]);
    beta[n] = inv_logit(mu_p[4] + sigma[4] * beta_pr[n]) * 10;
  }
}
model {
  // Hyperparameters
