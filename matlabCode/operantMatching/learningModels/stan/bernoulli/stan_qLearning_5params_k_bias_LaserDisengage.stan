...
// Declare all parameters as vectors for vectorizing
  ...
  vector[N] diff_pr;      // disengagement parameter
...
// Transform session-level raw parameters
  ...
  for (n in 1:N) {
    ...
    diff[n]   = Phi_approx(mu_p[5] + sigma[5] * diff_pr[n]);
  }
}
model {
  // Hyperparameters
  ...
  for (n in 1:N) {
    ...
    for (t in 1:(Tsesh[n])) {
      ...
      if (t > 1){
        if (laser[n, t-1] == 1) {
          choice[n,t] ~ bernoulli((1-diff[n])*inv_logit(beta[n] * Qdiff[t] + k[n] * prevChoice + bias[n]) + diff[n]*0.5);
        }else{
  ...
  { // local section, this saves time and space
    for (n in 1:N) {
      ...
      // compute log likelihood of current trial
        if (t > 1){
          if (laser[n, t-1] == 1){
            pRight  = (1-diff[n])*inv_logit(beta[n] * Qdiff[t] + k[n] * prevChoice + bias[n]) + diff[n]*0.5;
          }else{
            pRight = inv_logit(beta[n] * Qdiff[t] + k[n] * prevChoice + bias[n]);
          }
        }else{
          pRight = inv_logit(beta[n] * Qdiff[t] + k[n] * prevChoice + bias[n]);
        }

        log_lik[n] = log_lik[n] + bernoulli_lpmf(choice[n, t] | pRight);
