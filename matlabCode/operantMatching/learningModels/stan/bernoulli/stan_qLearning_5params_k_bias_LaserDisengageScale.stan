...
// Declare all parameters as vectors for vectorizing
  ...
  vector[N] scale_pr;      // disengagement parameter
...
// Transform session-level raw parameters
  ...
  for (n in 1:N) {
    ...
    scale[n]   = exp(mu_p[5] + sigma[5] * scale_pr[n]);
  }
}
model {
  // Hyperparameters
  ...
  for (n in 1:N) {
    ...
    for (t in 1:(Tsesh[n])) {
      ...
      if (t > 1){
        if (laser[n, t-1] == 1) {
          choice[n,t] ~ bernoulli_logit(scale[n] * beta[n] * Qdiff[t] + k[n] * prevChoice + bias[n]);
        }else{
  ...
  { // local section, this saves time and space
    for (n in 1:N) {
      ...
      // compute log likelihood of current trial
        if (t > 1){
          if (laser[n, t-1] == 1){
            log_lik[n] = log_lik[n] + bernoulli_logit_lpmf(choice[n, t] | scale[n]*beta[n] * Qdiff[t] + k[n] * prevChoice + bias[n]);;
          }else{
