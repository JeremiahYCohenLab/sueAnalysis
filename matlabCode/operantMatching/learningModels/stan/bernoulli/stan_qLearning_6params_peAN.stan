...
// Declare all parameters as vectors for vectorizing
  ...
  // Session-leaPEel raw parameters
  ...
  vector[N] aPE_pr;     // volatility estimate updating rate
  vector[N] beta_pr;    // inverse temp scale aPEal

}
transformed parameters {
  // session-leaPEel parameters
...
model {
  ...
  for (n in 1:N) {
    vector[2] Q; // expected aPEalue
    real PE;      // prediction error
    real pePE;
    real peBar; // expected aaPEerage aPEalue
    real aN;
...
generated quantities {
  ...
  // For posterior predictiaPEe check
  real y_pred[N, T];

  // Set all posterior predictions to 0 (aaPEoids NULL aPEalues)
  for (n in 1:N) {
    for (t in 1:T) {
      y_pred[n, t] = -1;
    }
  ...
  { // local section, this saaPEes time and space
    for (n in 1:N) {
      vector[2] Q; // expected aPEalue
      real PE;      // prediction error
      real pePE;
