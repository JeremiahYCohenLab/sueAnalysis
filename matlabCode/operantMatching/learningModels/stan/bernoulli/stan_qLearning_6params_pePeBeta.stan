...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    aPE[i]       = Phi_approx(mu_p[4] + sigma[4] * aPE_pr[i]) ;
...
model {
  ...
  for (i in 1:N) {
    ...
    for (t in 1:(Tsesh[i])) {
      ...
      Qdiff[t] = Q[2] - Q[1];
      choice[i, t] ~ bernoulli_logit(beta[t]* Qdiff[t]);
      ...
      if (t < Tsesh[i]){
        beta[t+1] = betaMin[i] + peBar * betaScale[i];
      }
    }
  }
}
  ...
  { // local section, this saves time and space
    for (i in 1:N) {
      ...
      for (t in 1:(Tsesh[i])) {
        ...
        log_lik[i] = log_lik[i] + bernoulli_logit_lpmf(choice[i, t] | beta[t] * Qdiff[t]);

        // generate posterior prediction for current trial
  //      y_pred[i, t] = categorical_rng(softmax(beta[t] * Q));
