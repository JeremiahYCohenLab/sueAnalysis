...
// Declare all parameters as vectors for vectorizing
  ...
  vector[N] betaScale_pr;  // inverse temperature
  vector[N] betaMin_pr;  // inverse temperature
  vector[N] rStart_pr;  // expected average reward starting value
}
transformed parameters {
  ...
  vector<lower=0, upper=1>[N] rStart;
  ...
  for (i in 1:N) {
    ...
    betaMin[i] = Phi_approx(mu_p[6] + sigma[6] * betaMin_pr[i]) * 20;
    rStart[i]    = Phi_approx(mu_p[7]  + sigma[7]  * rStart_pr[i]);
  }
}
model {
  // Hyperparameters
  ...
  for (i in 1:N) {
    ...
    for (t in 1:(Tsesh[i])) {
      ...
      choice[i, t] ~ bernoulli_logit(beta * Qdiff[t]);

      if (choice[i,t] == 1) {
        PE = outcome[i, t] - Q[2] - R;
        if (PE < 0){
          Q[2] = Q[2] + aN[i] * PE;
        }else{
          Q[2] = Q[2] + aP[i] * PE;
...
generated quantities {
  ...
  real<lower=0, upper=1> mu_rStart;
  ...
  { // local section, this saves time and space
    for (i in 1:N) {
      ...
      for (t in 1:(Tsesh[i])) {
        Qdiff[t] = Q[2] - Q[1];
        // compute log likelihood of current trial
        log_lik[i] += bernoulli_logit_lpmf(choice[i, t] | beta * Qdiff[t]);
