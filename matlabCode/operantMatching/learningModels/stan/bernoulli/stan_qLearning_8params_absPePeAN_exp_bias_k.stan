...
transformed parameters {
  ...
  vector<lower=0, upper=1>[N] aNa;
  vector<lower=0, upper=20>[N] aNb;
  ...
  for (n in 1:N) {
    aNa[n]  = Phi_approx(mu_p[1] + sigma[1] * aNa_pr[n]);
    aNb[n]  = Phi_approx(mu_p[2] + sigma[2] * aNb_pr[n]) * 20;
...
model {
  ...
  for (n in 1:N) {
    ...
    vector[Tsesh[n]] cP; // choice probabilities
    ...
    for (t in 1:(Tsesh[n])) {
      cP[t] = beta[n] * (Q[2] - Q[1]) + k[n] * prevChoice +  bias[n];

      if (choice[n,t] == 1) {
        PE = outcome[n, t] - Q[2];
        pePE = fabs(PE) - peBar;
        aN = aNa[n] * exp(pePE * aNb[n]);
        if (PE < 0){
          Q[2] += aN * PE;
        }else{
          Q[2] += aP[n] * PE;
    ...
    choice[n, 1:Tsesh[n]] ~ bernoulli_logit(cP);
  }
}
generated quantities {
  // For group level parameters
  real<lower=0, upper=1> mu_aNa;
  real<lower=0, upper=20> mu_aNb;
...
//  }
  ...
  mu_aNa  = Phi_approx(mu_p[1]);
  mu_aNb  = Phi_approx(mu_p[2]) * 20;
  ...
  { // local section, this saves time and space
    for (n in 1:N) {
      ...
      for (t in 1:(Tsesh[n])) {
        ...
        log_lik[n] += bernoulli_logit_lpmf(choice[n, t] | beta[n] * (Q[2] - Q[1]) + k[n] * prevChoice + bias[n]);
