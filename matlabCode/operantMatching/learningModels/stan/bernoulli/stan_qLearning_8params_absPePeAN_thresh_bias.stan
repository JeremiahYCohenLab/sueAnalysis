...
// Declare all parameters as vectors for vectorizing
  ...
  vector[N] thresh_pr;   // threshold for learning rate switch
  vector[N] bias;     // bias term
}
transformed parameters {
  // session-level parameters
  vector<lower=0, upper=1>[N] aNa;
  vector<lower=0, upper=1>[N] aNb;
  ...
  for (n in 1:N) {
    ...
    aNb[n]    = Phi_approx(mu_p[2] + sigma[2] * aNb_pr[n]);
    ...
    thresh[n] = mu_p[7] + sigma[7] * thresh_pr[n];
  }
}
model {
  // Hyperparameters
