...
transformed parameters {
  ...
  vector<lower=0, upper=10>[N] betaScale;
  ...
  for (n in 1:N) {
    ...
    betaScale[n]  = Phi_approx(mu_p[7] + sigma[7] * betaScale_pr[n]) * 10;
    betaMin[n]    = Phi_approx(mu_p[8] + sigma[8] * betaMin_pr[n]) * 10;
...
generated quantities {
  ...
  real<lower=0, upper=10> mu_betaScale;
  ...
  mu_betaScale = Phi_approx(mu_p[7]) * 10;
  mu_betaMin   = Phi_approx(mu_p[8]) * 10;
