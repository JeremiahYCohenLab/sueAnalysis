...
// Declare all parameters as vectors for vectorizing
  ...
  vector[N] kappa_pr;       //choice autoCorr
}
transformed parameters {
// Transform session-level raw parameters
  vector<lower=0, upper=1>[N] lambda;
  real<lower=0, upper=20> vInit;
  vector<lower=0, upper=20>[N] omega;
  vector<lower=0, upper=1>[N] beta;
  vector<lower=0, upper=5>[N] kappa;
  
  for (n in 1:N) {
    lambda[n]   = inv_logit(mu_p[1] + sigma[1] * lambda_pr[n])*0.2;
    omega[n]   = inv_logit(mu_p[3] + sigma[3] * omega_pr[n])*20;
    beta[n] = inv_logit(mu_p[4] + sigma[4] * beta_pr[n]);
    kappa[n] =  inv_logit(mu_p[5] + sigma[5] * kappa_pr[n])*5;
  }

  vInit = inv_logit(mu_p[2])*20;
}
model {
  // Hyperparameters
  mu_p[1]  ~ normal(0, 3);
  ...
  for (n in 1:N) {
    ...
    for (t in 1:(Tsesh[n])) {
      ...
      choice[n, t] ~ bernoulli_logit(beta[n] * mdiff[t] + kappa[n]*prevChoice);
...
generated quantities {
  ...
  real<lower=0, upper=0.2> mu_lambda;
  real<lower=0, upper=20> mu_vInit;
  real<lower=0, upper=20> mu_omega;
  real<lower=0, upper=1> mu_beta;
  real<lower=0, upper=5> mu_kappa;

  // For log likelihood calculation
  real log_lik[N];

  mu_lambda  = inv_logit(mu_p[1])*0.2;
  mu_vInit   = inv_logit(mu_p[2])*20;
  mu_omega   = inv_logit(mu_p[3])*20;
  mu_beta    = inv_logit(mu_p[4]);
  mu_kappa   = inv_logit(mu_p[5])*5;
  ...
  { // local section, this saves time and space
    for (n in 1:N) {
      ...
      for (t in 1:(Tsesh[n])) {
        mdiff[t] = m[t,2] - m[t,1];
        log_lik[n] += bernoulli_logit_lpmf(choice[n, t] | (beta[n] * mdiff[t] + kappa[n]*prevChoice));
