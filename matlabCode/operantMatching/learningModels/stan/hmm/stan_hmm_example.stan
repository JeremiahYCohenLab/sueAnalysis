...
parameters {
  ...
  vector[K] mu;                    // observation means, not use ordered because it affects sampling
...
model {
  ...
  // sigma ~ cauchy(0,1);
  mu[1] ~ normal(10,1);
  matrix[T,K] logalpha;  // print(mu);
  // print(sigma);

  { // Forward algorithm log p(z_t = j | x_{1:t})
    real accumulator[K];
    for (j in 1:K){
      logalpha[1,j] = log(pi1[j]) + normal_lpdf(y[1] | mu[j], sigma[j]);
    }
...
//     for (j in 1:K)
//       delta[1, j] = normal_lpdf(y[1] | mu[j], sigma[j]);
