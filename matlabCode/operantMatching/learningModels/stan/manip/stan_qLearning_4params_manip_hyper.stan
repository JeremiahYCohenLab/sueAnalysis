...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    beta[i] = 10*Phi_approx(mu_p[4] + sigma[4] * beta_pr[i]);
    }
  for (i in 1:M){
    ...
    d_beta[i] = 10*Phi_approx(d_mu_p[4]  + d_sigma[4]  * d_beta_pr[i]);
  }
}
model {
  //hyper-hyperparameters
  ...
  for (i in 1:M) {
    ...
    for (t in 1:(TseshM[i])) {
      // compute action probabilities
      choiceM[i, t] ~ categorical_logit(d_beta[i] * Q);
...
generated quantities {
  ...
  { // local section, this saves time and space
    ...
    for (i in 1:M) {
      ...
      for (t in 1:(TseshM[i])) {
        // compute log likelihood of current trial
        log_likM[i] = log_likM[i] + categorical_logit_lpmf(choiceM[i, t] | d_beta[i] * Q);

        // generate posterior prediction for current trial
        y_predM[i, t] = categorical_rng(softmax(d_beta[i] * Q));
