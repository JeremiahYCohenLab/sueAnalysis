...
transformed parameters {
  ...
  for (i in 1:M){
    d_aN[i]   = -1 + Phi_approx(d_mu_p[1]  + d_sigma[1]  * d_aN_pr[i]);
    d_aP[i]   = -1 + Phi_approx(d_mu_p[2]  + d_sigma[2]  * d_aP_pr[i]);
    d_aF[i]   = -1 + Phi_approx(d_mu_p[3]  + d_sigma[3]  * d_aF_pr[i]);
    d_beta[i] = -10 + 10*Phi_approx(d_mu_p[4]  + d_sigma[4]  * d_beta_pr[i]);
  }
}
model {
  // Hyperparameters
