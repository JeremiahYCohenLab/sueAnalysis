...
transformed parameters {
  ...
  for (i in 1:M){
    ...
    d_v[i]    = -1 + 2*Phi_approx(d_mu_p[4] + d_sigma[4] * d_v_pr[i]);
...
model {
  ...
  for (i in 1:M) {
    ...
    for (t in 1:(TseshM[i])) {
      ...
      R = (v[i] + d_v[i]) * outcomeM[i, t] + (1 - (v[i] + d_v[i])) * R;
...
generated quantities {
  ...
  real<lower=0, upper=10> mu_betaDiff;
  real<lower=-1, upper=1> d_mu_aN;
  real<lower=-1, upper=1> d_mu_aP;
  real<lower=-1, upper=1> d_mu_aF;
  real<lower=-1, upper=1> d_mu_v;
  real<lower=-10, upper=10> d_mu_betaMin;
  real<lower=-10, upper=10> d_mu_betaDiff;
  ...
  mu_betaMin = Phi_approx(mu_p[5]) * 10;
  mu_betaDiff = Phi_approx(mu_p[6]) * 10;

  d_mu_aN   = -1 + 2*(Phi_approx(d_mu_p[1]));
  d_mu_aP   = -1 + 2*(Phi_approx(d_mu_p[2]));
  d_mu_aF   = -1 + 2*(Phi_approx(d_mu_p[3]));
  d_mu_v   = -1 + 2*(Phi_approx(d_mu_p[4]));
  d_mu_betaMin = -10 + 20*(Phi_approx(d_mu_p[5]));
  d_mu_betaDiff = -10 + 20*(Phi_approx(d_mu_p[6]));
