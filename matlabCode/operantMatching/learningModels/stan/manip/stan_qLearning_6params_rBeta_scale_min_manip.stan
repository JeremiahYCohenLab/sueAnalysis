...
// Declare all parameters as vectors for vectorizing
  ...
  vector[N] betaMin_pr;  // inverse temperature for softmax (decision) function
  ...
  vector[M] d_betaMin_pr;  // change in inverse temperature
}
transformed parameters {
  ...
  vector<lower=0, upper=20>[M] d_betaMin;
  ...
  for (i in 1:N) {
    ...
    betaMin[i] = 20*Phi_approx(mu_p[6] + sigma[6] * betaMin_pr[i]);
    }
  for (i in 1:M){
    ...
    d_betaMin[i] = 20*Phi_approx(d_mu_p[6] + d_sigma[6] * d_betaMin_pr[i]);
  }
}
model {
  //hyper-hyperparameters
  ...
  for (i in 1:M) {
    ...
    for (t in 1:(TseshM[i])) {
      ...
      beta = R * d_betaScale[i] + d_betaMin[i];
    }
  }
}
generated quantities {
  ...
  real<lower=0, upper=20> d_mu_betaMin;
  ...
  mu_betaMin = 20*Phi_approx(mu_p[6]);
  ...
  d_mu_betaMin = 20*(Phi_approx(d_mu_p[6]));
