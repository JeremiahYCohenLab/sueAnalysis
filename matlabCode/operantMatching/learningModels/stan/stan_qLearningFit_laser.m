function [paramEsts] = stan_qLearningFit_laser(xlFile, sheet, category, varargin)
...
if p.Results.bernFlag
    if p.Results.shuffle
        savePath = [root sheet sep sheet 'sorted' sep 'stan' sep 'bernoulli' sep 'compare' sep p.Results.modelName sep paramNames{p.Results.cmpParam} sep category sep 'shuffle' sep];
    else
