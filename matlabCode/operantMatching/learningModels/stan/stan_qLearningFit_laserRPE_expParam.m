...
session_dat = struct('N',N,'T',T, 'S', S, 's', s, 'Tsesh', Tsesh, 'choice', choice, 'outcome', outcome, 'ITI', ITI, 'laser', laser);
...
for currAni = 1:S
    for currP = 1:length(paramNames)
        subplot(S, numParams, numParams*(currAni-1)+currP); hold on;
        mu = tmpSamp.([paramNames{currP}])(:,currAni);
        ...
        p5__ = summary{{[paramNames{currP} '[' num2str(currAni) ']']}, {'p5_'}};
        p95__ = summary{{[paramNames{currP} '[' num2str(currAni) ']']}, {'p95_'}};
...
% pull all animals together
%%
figure; hold on;

allDiff = tmpSamp.(paramNames{end});
% allDiff = allDiff(tmpSamp.divergent__==0,:);
% mu = tmpSamp.([paramNames{currP}])(:,currAni);
minDiff = min(allDiff, [], 'all');
maxDiff = max(allDiff, [], 'all');

edges = linspace(minDiff-0.00001, maxDiff+0.00001, 50);
midPoints = 0.5*(edges(1:end-1) + edges(2:end));
for ani = 1:S
    counts = histcounts(allDiff(:, ani), edges);
    counts = counts/(sum(counts));
    horiLevel = (ani-1)*0.15;
    patch([midPoints, flip(midPoints)], [zeros(size(counts)), flip(counts)]+horiLevel, 'm', 'FaceAlpha', 0.4, 'edgeColor', 'none');
%     histogram(allDiff(:,ani), 'FaceColor', 'm', 'EdgeColor', 'none');
    p5__ = summary{{[paramNames{end} '[' num2str(ani) ']']}, {'p5_'}};
    p95__ = summary{{[paramNames{end} '[' num2str(ani) ']']}, {'p95_'}};
    scatter([p5__ p95__], ones(1, 2)*horiLevel, 15, 'r', 'filled')

end

plot([1 1], [0 ani*0.15], 'LineStyle', '--', 'LineWidth', 2, 'Color', [0.5 0.5 0.5]);
%%
% samps = tmpSamp;
% dFig = figure;
