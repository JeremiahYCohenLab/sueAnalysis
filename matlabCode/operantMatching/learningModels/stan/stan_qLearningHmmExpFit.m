function [paramEsts] = stan_qLearningHmmExpFit(xlFile, sheet, category, varargin)
...
%         fprintf([sessionName '\n']);
    ...
    emisGuess = [0.5 0.5;
                 1 0;
                 0 1];
    hmmStates = cell(length(dayList),1);
    [estTR,estE] = hmmtrainPrior(choiceTmpHmm,transGuess,emisGuess);
    for i = 1:length(dayList)      
        hmm = hmmviterbi(choiceTmpHmm{i}, estTR, estE);
%         transEInd = find(hmm(2:end-1)==1 & hmm(1:end-2)~=hmm(3:end) & hmm(1:end-2)~= 1 & hmm(3:end)~= 1)+1;
%         hmm(transEInd) = 0;
        hmm(hmm~=1) = 0;
        hmmStates{i} = hmm;
    end
