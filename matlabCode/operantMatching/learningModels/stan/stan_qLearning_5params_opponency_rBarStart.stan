...
// Declare all parameters as vectors for vectorizing
  ...
  vector[N] rBarS_pr;   // expected average value start value
}
transformed parameters {
  ...
  vector<lower=0, upper=1>[N] rBarS;
  ...
  for (i in 1:N) {
    ...
    rBarS[i]   = Phi_approx(mu_p[6]  + sigma[6]  * rBarS_pr[i]);
  }
}
model {
  // Hyperparameters
