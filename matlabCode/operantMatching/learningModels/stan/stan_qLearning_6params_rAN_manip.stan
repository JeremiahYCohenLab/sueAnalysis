...
// Declare all parameters as vectors for vectorizing
  ...
  vector[N] aNscale_pr;    // learning rate for negative prediction error (NPE)
  vector[N] aNmin_pr;    // learning rate for negative prediction error (NPE)
  ...
  vector[M] d_aNscale_pr;    // learning rate for negative prediction error (NPE)
  vector[M] d_aNmin_pr;    // learning rate for negative prediction error (NPE)
...
transformed parameters {
  ...
  vector<lower=0, upper=10>[N] aNscale;
  ...
  vector<lower=0, upper=10>[M] d_aNscale;
  vector<lower=0, upper=1>[M] d_aNmin;
  vector<lower=0, upper=1>[M] d_aP;
  vector<lower=0, upper=1>[M] d_aF;
  vector<lower=0, upper=1>[M] d_v;
  vector<lower=0, upper=20>[M] d_beta;
  ...
  for (i in 1:N) {
    aNscale[i] = Phi_approx(mu_p[1] + sigma[1] * aNscale_pr[i]) * 10;
  ...
  for (i in 1:M){
    d_aNscale[i] = Phi_approx(d_mu_p[1] + d_sigma[1] * d_aNscale_pr[i]) * 10;
    d_aNmin[i]   = Phi_approx(d_mu_p[2] + d_sigma[2] * d_aNmin_pr[i]);
    d_aP[i]      = Phi_approx(d_mu_p[3] + d_sigma[3] * d_aP_pr[i]);
    d_aF[i]      = Phi_approx(d_mu_p[4] + d_sigma[4] * d_aF_pr[i]);
    d_v[i]       = Phi_approx(d_mu_p[5] + d_sigma[5] * d_v_pr[i]);
    d_beta[i]    = Phi_approx(d_mu_p[6] + d_sigma[6] * d_beta_pr[i]) * 20;
  }
}
model {
  //hyper-hyperparameters
  ...
  for (i in 1:M) {
    ...
    for (t in 1:(TseshM[i])) {
      // compute action probabilities
      choiceM[i, t] ~ categorical_logit(beta[i] * Q);
      ...
      // value updating (learning)
      if (PE < 0){
        Q[choiceM[i, t]] = Q[choiceM[i, t]] + aN * PE;
      }
      else{
      ...
      R = d_v[i] * outcome[i, t] + (1-d_v[i]) * R;
      aN = R * d_aNscale[i] + d_aNmin[i];
    }
  }
}
generated quantities {
  // For group level parameters
  real<lower=0, upper=10> mu_aNscale;
  ...
  real<lower=0, upper=10> d_mu_aNscale;
  real<lower=0, upper=1> d_mu_aNmin;
  real<lower=0, upper=1> d_mu_aP;
  real<lower=0, upper=1> d_mu_aF;
  real<lower=0, upper=1> d_mu_v;
  real<lower=0, upper=20> d_mu_beta;
  ...
  mu_aNscale  = Phi_approx(mu_p[1]) * 10;
  ...
  d_mu_aNscale  = Phi_approx(d_mu_p[1]) * 10;
  d_mu_aNmin    = Phi_approx(d_mu_p[2]);
  d_mu_aP       = Phi_approx(d_mu_p[3]);
  d_mu_aF       = Phi_approx(d_mu_p[4]);
  d_mu_v        = Phi_approx(d_mu_p[5]);
  d_mu_beta     = Phi_approx(d_mu_p[6]) * 20;
  ...
  { // local section, this saves time and space
    ...
    for (i in 1:M) {
      ...
      for (t in 1:(TseshM[i])) {
        // compute log likelihood of current trial
        log_likM[i] = log_likM[i] + categorical_logit_lpmf(choiceM[i, t] | beta[i] * Q);

        // generate posterior prediction for current trial
        y_predM[i, t] = categorical_rng(softmax(beta[i] * Q));
