...
// Declare all parameters as vectors for vectorizing
  ...
  vector[N] betaDiff_pr;   // inverse temp max-min val
...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    betaDiff[i] = Phi_approx(mu_p[6] + sigma[6] * betaDiff_pr[i]) * 20;
  }
}
model {
  // Hyperparameters
...
generated quantities {
  ...
  real<lower=0, upper=20> mu_betaDiff;
  ...
  mu_betaDiff = Phi_approx(mu_p[6]) * 20;
