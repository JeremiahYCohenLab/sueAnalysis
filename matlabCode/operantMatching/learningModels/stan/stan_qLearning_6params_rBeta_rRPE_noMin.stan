...
// Declare all parameters as vectors for vectorizing
  ...
  vector[N] rRate_pr;  // inverse temperature updating rate
  vector[N] betaStart_pr;   // inverse temp min val
  vector[N] betaScale_pr;   // inverse temp max-min val
...
transformed parameters {
  ...
  vector<lower=0, upper=20>[N] betaStart;
  ...
  for (i in 1:N) {
    ...
    betaStart[i] = Phi_approx(mu_p[5] + sigma[5] * betaStart_pr[i]) * 20;
    betaScale[i] = Phi_approx(mu_p[6] + sigma[6] * betaScale_pr[i]) * 20;
  }
}
model {
  // Hyperparameters
...
generated quantities {
  ...
  real<lower=0, upper=20> mu_rRate;
  real<lower=0, upper=20> mu_betaStart;
  ...
  mu_rRate = Phi_approx(mu_p[4]); 
  mu_betaStart = Phi_approx(mu_p[5]) * 20;
  mu_betaScale = Phi_approx(mu_p[6]) * 20;
