...
// Declare all parameters as vectors for vectorizing
  ...
  vector[N] initR_pr;    // inverse temp init val
...
transformed parameters {
  ...
  vector<lower=0, upper=1>[N] initR;
  ...
  for (i in 1:N) {
    ...
    initR[i] = Phi_approx(mu_p[6] + sigma[6] * initR_pr[i]);
  }
}
model {
  // Hyperparameters
...
generated quantities {
  ...
  real<lower=0, upper=20> mu_initR;
