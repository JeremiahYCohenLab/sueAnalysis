...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    k[i] = Phi_approx(mu_p[6] + sigma[6] * k_pr[i]) ;
  }
}
model {
  // Hyperparameters
  ...
  for (i in 1:N) {
    ...
    for (t in 1:(Tsesh[i])) {
      ...
      choice[i, t] ~ categorical_logit(beta * Q + k[i]*prevChoice);
      // prediction error
...
generated quantities {
  ...
  { // local section, this saves time and space
    for (i in 1:N) {
      ...
      for (t in 1:(Tsesh[i])) {
        // compute log likelihood of current trial & generate posterior prediction
        log_lik[i] = log_lik[i] + categorical_logit_lpmf(choice[i, t] | beta * Q + k[i] * prevChoice);
        y_pred[i, t] = categorical_rng(softmax(beta * Q + k[i] * prevChoice));
