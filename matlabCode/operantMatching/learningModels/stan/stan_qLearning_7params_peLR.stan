...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    aPscale[i]   = Phi_approx(mu_p[3]  + sigma[3]  * aPscale_pr[i]);
    aPmin[i]   = Phi_approx(mu_p[4]  + sigma[4]  * aPmin_pr[i]);
    aF[i]   = Phi_approx(mu_p[5]  + sigma[5]  * aF_pr[i]);
    v[i] = Phi_approx(mu_p[6] + sigma[6] * v_pr[i]) ;
    beta[i] = Phi_approx(mu_p[7] + sigma[7] * beta_pr[i]) * 20;
  }
}
model {
  // Hyperparameters
