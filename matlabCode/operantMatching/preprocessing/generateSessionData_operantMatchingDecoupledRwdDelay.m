...
for i = 1:length(sessionText)
    ...
    if regexp(sessionText{i},'L Trial ') % trial begin 
        ...
        tCSFlag = false;
        j = tBegin + 1; % start looking for CS on
        while (~tCSFlag) 
            if regexp(sessionText{j},'CS ')
                tCS = j; 
                tCSFlag = true;
            else
                j = j + 1;
        ...
        tEndFlag = false;
        j = tCS + 1; % start looking for last index of trial
        while (~tEndFlag) 
            if regexp(sessionText{j},'CS ')
                tEnd = j - 2; 
                tEndFlag = true;
        ...
        if strfind(sessionText{tCS},'CS PLUS')
           behSessionData(currTrial).trialType = 'CSplus';
           temp = regexp(sessionText(tCS), ': ', 'split');
           behSessionData(currTrial).CSon = str2double(temp{1}{2});
        elseif strfind(sessionText{tCS},'CS MINUS')
            behSessionData(currTrial).trialType = 'CSminus';
            temp = regexp(sessionText(tCS), ': ', 'split');
            behSessionData(currTrial).CSon = str2double(temp{1}{2});
        end
        
        for currTrialInd = tBegin: tCS
        ...
        if regexp(sessionText{currTrialInd},'R Block Switch at Trial ')
            if currTrial ~= 1
                blockSwitch = [blockSwitch currTrial+1];
                blockSwitchR = [blockSwitchR currTrial+1];
            end
        end
        end
