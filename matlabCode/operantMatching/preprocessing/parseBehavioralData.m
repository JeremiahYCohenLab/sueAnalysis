function outputStruct = parseBehavioralData(sessionData, blockSwitch)

responseInds = find(~isnan([sessionData.rewardTime]));
omitInds = isnan([sessionData.rewardTime]);
blockSwitch = blockSwitch(blockSwitch<length(sessionData));
...
timeBtwn = [sessionData(responseInds(2:end)).rewardTime] - [sessionData(responseInds(1:end-1)).CSon];
