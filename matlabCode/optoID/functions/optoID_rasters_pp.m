function summary = optoID_rasters_pp(sessionsToImport, delayComp, TTdelay, CSCdelay, varargin)

root = currComputer_pP;
...
% end
if exist(strcat(root, animalName, '\', sessionName, '\sorted\laserStruct.mat'),'file')
    load(strcat(root, animalName, '\', sessionName, '\sorted\laserStruct.mat'))
else
    [~, ~, laserStruct] = generateTrialStruct_uU(sessionsToImport);
    save(strcat(root, animalName, '\', sessionName, '\sorted\laserStruct.mat'), 'laserStruct')
end


if delayComp == 0 % this compensation is for DSP delay compensation disabled sessions
    TTdelay = 0; % will typically be 2468
    CSCdelay = 0; % will typically be 484
    spikeDelay = 0;
elseif delayComp == 1
...
saveDir = [root animalName '\' sessionName '\figures'];
if ~isfolder(saveDir)
    mkdir(saveDir)
end
...
%                 spikeTimes_light_TT = [spikeTimes_light_TT tempSpike];
    ...
    spikeTimes_light_TT = 1000*spikeTimes_light_TT - spikeDelay;
    if ~isempty(spikeTimes_light_TT)
        [spikeTimes_light_CSC, spikeCSC_light] = Nlx2MatCSC(strcat(root, animalName, '\', sessionName, '\neuralynx\', CSCchann{1},'.ncs'),[1 0 0 0 1],0,5,spikeTimes_light_TT);
        ...
        [spikeCSC_light(:,:,2)] = Nlx2MatCSC(strcat(root, animalName, '\', sessionName, '\neuralynx\', CSCchann{2},'.ncs'),[0 0 0 0 1],0,5,spikeTimes_light_TT);
        [spikeCSC_light(:,:,3)] = Nlx2MatCSC(strcat(root, animalName, '\', sessionName, '\neuralynx\', CSCchann{3},'.ncs'),[0 0 0 0 1],0,5,spikeTimes_light_TT);
        [spikeCSC_light(:,:,4)] = Nlx2MatCSC(strcat(root, animalName, '\', sessionName, '\neuralynx\', CSCchann{4},'.ncs'),[0 0 0 0 1],0,5,spikeTimes_light_TT);
        spikeInd_light = floor((spikeTimes_light_TT - spikeTimes_light_CSC)/31.25);
    end

    spikeTimes_spont_TT = 1000*spikeTimes_spont_TT - spikeDelay;
    [spikeTimes_spont_CSC, spikeCSC_spont] = Nlx2MatCSC(strcat(root, animalName, '\', sessionName, '\neuralynx\', CSCchann{1},'.ncs'),[1 0 0 0 1],0,5,spikeTimes_spont_TT);
    ...
    [spikeCSC_spont(:,:,2)] = Nlx2MatCSC(strcat(root, animalName, '\', sessionName, '\neuralynx\', CSCchann{2},'.ncs'),[0 0 0 0 1],0,5,spikeTimes_spont_TT);
    [spikeCSC_spont(:,:,3)] = Nlx2MatCSC(strcat(root, animalName, '\', sessionName, '\neuralynx\', CSCchann{3},'.ncs'),[0 0 0 0 1],0,5,spikeTimes_spont_TT);
    [spikeCSC_spont(:,:,4)] = Nlx2MatCSC(strcat(root, animalName, '\', sessionName, '\neuralynx\', CSCchann{4},'.ncs'),[0 0 0 0 1],0,5,spikeTimes_spont_TT);
    ...
    rasters = figure; subplot(4,3,[1:6]); hold on; title(strcat(spikeFields{i},' ;  ', sessionName),'Interpreter','none')
    xlabel('Time (ms)'); ylabel('Trials')
    LineFormat.Color = 'k'; LineFormat.LineWidth = 1;
    if ~all(cellfun(@isempty, spikeRast)) % only if there are spikes
    ...
    for currCSC = 1:4
        ...
        xlabel('Time (ms)'); ylabel('Amplitude (uV)')
        if ~isempty(lightSpikeWaveforms)
            if size(lightSpikeWaveforms, 2) > 1
        ...
        if ~isempty(spontSpikeWaveforms)
            if size(spontSpikeWaveforms, 2) > 1
...
%     saveFigurePDF(rasters,[saveDir '\optoID_' spikeFields{i}])
    ...
    summary.(spikeFields{i}).lat = latencyToResponse;
    summary.(spikeFields{i}).prob = numResponses;
    tmpSpikes = laserStruct.(spikeFields{i});
    summary.(spikeFields{i}).fr = numel(tmpSpikes) / (tmpSpikes(end) - tmpSpikes(1)) * 1000;
end