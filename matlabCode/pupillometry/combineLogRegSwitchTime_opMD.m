function [glm_rwdLick, stayLickLat, switchLickLat, binSize, timeMax, combinedITIlicks] = combineLogRegSwitchTime_opMD(animal, category, varargin)
...
for i = 1: length(dayList)
    ...
    %%generate matrix for tMax trials
 ...
 %% nrwd matrix
    ...
    for j = 2:length(responseInds)          
        ...
        if ~isempty(timeTmp)
            ...
            for k = 1:tMax
                if ~isempty(find(binnedRwds == k, 1))
                    nrwdTmpMatx(k,j) = sum(binnedRwds == k);
                end
            end
        end
    end
    ...
    while behSessionData(responseInds(j)).rewardTime - behSessionData(responseInds(1)).rewardTime < timeMax
        tmpDiff = behSessionData(responseInds(j)).rewardTime - behSessionData(responseInds(1)).rewardTime;
        binnedDiff = discretize(tmpDiff, timeBinEdges);
        nrwdTmpMatx(binnedDiff:tMax,j) = NaN;
        j = j+1;
    end
    %concatenate temp matrix with combined matrix
    nrwdTmpMatx(:,1) = NaN;
    nrwdMatx = [nrwdMatx NaN(length(timeBinEdges)-1, 100) nrwdTmpMatx];
...
%%
preITIs = 0.001 * preITIs;

%regression model
glm_all = fitglm([rwdMatx',nrwdMatx', preITIs'], eve, 'distribution','binomial','link','logit');
...
if p.Results.plotFlag
    ...
    errorL = abs(coefVals - CIbands(relevInds,1));
    errorU = abs(coefVals - CIbands(relevInds,2));
    errorbar(((1:tMax)*binSize/1000),coefVals,errorL,errorU,'Color', [0 1 1],'linewidth',2)
    
    xlabel('rwd/nrwd n seconds back')
    ylabel('\beta Coefficient')
    xlim([0 (tMax*binSize/1000 + 5)])
    line([0 (tMax*binSize/1000 + 5)], [0 0], 'Color','k','LineStyle','--')
