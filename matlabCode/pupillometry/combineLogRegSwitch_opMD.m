function [glm_seshInd, tMax] = combineLogRegSwitch_opMD(animal, category, revForFlag, plotFlag)
...
%% ITI
    ...
    combinedpreITIs = [combinedpreITIs NaN(1, 100) preITItemp];
%%
...
combinedpreITIs = 0.0005 * combinedpreITIs;
...
glm_rwdNoRwd = fitglm(abs([combinedNoRewardsMatx' combinedpreITIs']), combinedeve,'distribution','binomial','link','logit'); rsq{7} = num2str(round(glm_rwdNoRwd.Rsquared.Adjusted*100)/100);
...
if plotFlag
    ...
    errorbar((1:tMax)+0.2,coefVals,errorL,errorU,'Color', [0.2 0.2 1],'linewidth',2)
    line([0.5 tMax+0.5], [0 0], 'Color','k','LineStyle','--')
    bar(tMax+2.5,glm_rwdNoRwd.Coefficients.Estimate(end), 'FaceColor',[0.2 0.2 1]);
    errorbar(tMax+2.5,glm_rwdNoRwd.Coefficients.Estimate(end), abs(glm_rwdNoRwd.Coefficients.Estimate(end) - CIbands(end,1)), abs(glm_rwdNoRwd.Coefficients.Estimate(end) - CIbands(end,2)),'Color', [0.4 0.4 0.4],'linewidth',1)
