function [errorProp,csFT, qualInd, ratioMax] = timeAlignCG(session, plotFlag, saveFlag)
...
while length(error) > errorRate*length(csT) && iter < iterMax
    ...
    for a = 1:length(errorSeq)
        if (sum(ledLL(csFT(error(errorSeq(a))) : csFT(error(errorSeq(a))) + 5)) > 0) && min(abs(error(errorSeq(a))+1 - startTrial))>5
...
pupilInds = pupilInds;% convert from csplus indexing to all trial type indexing 
