function pupilCompareInh(xlFile, sheet, col1, col2, varargin)
%task and model parameters
p = inputParser;
% default parameters if none given
p.addParameter('controlCat', 'allcontrol');
p.addParameter('inhiCat', 'inhibition');
...
%% combine all animals from same category
[root, sep] = currComputer();
% control
[~ , control, ~] = xlsread([root xlFile], sheet);
[~,col] = find(contains(control, col1));
control = control(2:end,col);
endInd = find(cellfun(@isempty,control),1);
if ~isempty(endInd)
    control= control(1:endInd-1,:);
end
baselinesC = nans(length(control), 7);
combinePupilC = [];
combineQualIndsC = [];
combineRwdC = [];
combineRwdHxC = [];
...
for i = length(control)
    ...
    % collect information together and combine
    % baseline diameter before the 1st trial, 1-50 50-100 ...
    for win = 1:size(baselinesC,2)
        if win == 1
            baselineC(i,win) = baseline(1);
        else
            baselineC(i,win) = mean(baseline(qualInd(50*(win-2)+1:min(50*(win-1), length(dilation)))));
        end
    end
      stab
    % also collect actual matrix for GLM analysis
    combinePupilC = [combinePupilC; sessionPupilCue];
    % rwd info, rwd his, switch/stay, pre rwd/stay, qualInds
    rwdTemp = zeros(1,length(os.responseInds));
    rwdTemp(os.rwd_Inds) = 1;
    swTemp = zeros(1,length(os.responseInds));
    swTemp(os.changeChoice_Inds) = 1;
    respIndsTemp = zeros(1,length(dilation));
    respIndsTemp(os.responseInds) = 1;
    
    combineRwdC = [combineRwdC, rwdTemp];
    combineSwitchC = [combineSwitchC, swTemp];
    combineRwdHxC = [combineRwdHxC, os.rwdHx];
    combineQualIndsC = [combineQualIndsC, qualInds];
    combineResponseInds = [combineResponseInds, respIndsTemp];
    
end
% inhibition
[~ , inhibition, ~] = xlsread([root xlFile], sheet);
[~,col] = find(contains(inhibition, col2));
inhibition = inhibition(2:end,col);
endInd = find(cellfun(@isempty,inhibition),1);
if ~isempty(endInd)
    inhibition= inhibition(1:endInd-1,:);
end
...
dilation = max(sessionPupilCue(:,round(2*FR)+1:end),[],2,'omitnan')-baseline;
subplot(6,8,17)
scatter(baseline(qualInd),dilation(qualInd),6,'c','filled');
...
%% autoCorrlation
subplot(6,8,[19:21])
autocorr(baseline(qualInd),'NumLags',50,'NumSTD',3);
title('baseline autoCorr')
xlabel('lags in trial')
subplot(6,8,[22:24]);
autocorr(dilation(qualInd),'NumLags',50,'NumSTD',3);
