...
ani = 'all';
col = 'activationLow';
dayList = getDayList(file, ani, col);
...
%     lenL(i) = length(mintersect(os.rwd_Inds+1, laserNrwdInd, os.stayChoice_Inds));
    ...
    switchCurrControl(i) = sum(svs(os.laser==0), 'omitmissing')/sum(os.laser==0);
    switchCurrLaser(i) = sum(svs(os.laser==1), 'omitmissing')/sum(os.laser==1);
...
title(['pSwitch|nrwd' num2str(p,3)], 'FontSize', 13)
xlabel('Control', 'FontSize', 18)
ylabel('Laser', 'FontSize', 18)
