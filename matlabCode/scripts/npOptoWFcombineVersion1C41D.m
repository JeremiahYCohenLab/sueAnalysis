...
[coeff,score,latent, ~, explained, mu] = pca(allWFzs(~isnan(allWFzs(:,1)),:));
...
locTemp = loc(allWFs(:,21)<-40);
%%
allWFsTemp = allWFs(allWFs(:,21)<-40, :);
allWFsTemp = allWFsTemp./allWFsTemp(:,21);
meanWF = mean(allWFsTemp, 1, 'omitmissing');
stdWF = std(allWFsTemp, 0, 1, 'omitmissing');

allWFzs = (allWFsTemp - meanWF)./stdWF;
allWFzs = allWFzs(:, [1:20, 22:end]);
%%
...
hold on
for i = 1:numCat
scatter3(score(ind==i,1), score(ind==i,2), score(ind==i,3))
end
figure2;
hold on
for i = 1:numCat
    plot(mean(allWFsTemp(ind==i, :)))
end

figure;
hold on
locTemp = loc(allWFs(:,21)<-40);
edges = linspace(min(locTemp)-0.01, max(locTemp)+0.01, 15);
for i = 1:numCat
    histogram(locTemp(ind==i), edges, 'Normalization', 'probability')
end
...
locTemp = locTemp;
locNew = locTemp;
allSessionsTemp = allSessions(allWFs(:,21)<-40);
%%
for i = 1:length(dayList)

    % fprintf(num2str(i))
    currLoc = locTemp(allSessionsTemp == i);
    currInd = ind(allSessionsTemp == i);

    tempCat = currInd;
    tempCat(currInd == 1 | currInd == 4) = 0;
    tempCat(currInd == 2 | currInd == 3) = 1;
...
load('F:\tmpData\pcaCoeff.mat', 'coeff');
x = (-20:39)/30;
xq = (-20:40)/32;
waveformsInt = zeros(size(allWFsTemp,1), length(xq));
for i = 1:size(allWFsTemp,1)
    waveformsInt(i,:) = interp1(x, allWFsTemp(i,:), xq);
end
%% zscore
waveformsIntzs = zscore(waveformsInt, [], 1);

score = waveformsIntzs * coeff(11:71,:);
%%