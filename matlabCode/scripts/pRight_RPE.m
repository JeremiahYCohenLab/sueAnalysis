...
%% shape back
peCombined = reshape(peCombined, maxTrial+10, []);
outcomeCombined = reshape(outcomeCombined, maxTrial+10, []);
choiceCombined = reshape(choiceCombined, maxTrial+10, []);
predRCombined = reshape(predRCombined, maxTrial+10, []);
predCCombined = reshape(predCCombined, maxTrial+10, []);

peCombined = peCombined(1:maxTrial, :)';
outcomeCombined = outcomeCombined(1:maxTrial, :)';
choiceCombined = choiceCombined(1:maxTrial, :)';
...
parfor i = 1:size(peCombined, 1)
    choices = choiceCombined(i, :);
    rpe = peCombined(i, :);
    omitInd = abs(rpe)>2;
    ...
    stay(omitInd) = NaN;
    % permutation
    permInds = randperm(length(stay))';
    stay = stay(permInds);
    % pStay = predRCombined(i,:);
    % pStay(find(choices(1:end-1)==0)+1) = 1 - pStay(find(choices(1:end-1)==0)+1);
    % pStay(1) = NaN;
    pChoicePred = predRCombined(i,:);
    pChoicePred(choices==0) = 1 - pChoicePred(choices==0);
    meanChange = mean(stay(2:end)' - pChoicePred(1:end-1), 'omitmissing');
...
xlabel('rpe');
ylabel('simulation deltaP(currChoice)')
set(gca,'Box', 'off');
set(gca, 'TickDir', 'out')
...
set(gca,'Box', 'off');
set(gca, 'TickDir', 'out')
%% plot predicted pStay-rpe (can be borrowed by simulation) permutation
rpe = peCombined;
% rightRpe(choiceCombined==0) = -rightRpe(choiceCombined==0);
...
for i = 1:length(allChoices)
    ...
    for j = 1:numBins
        currInd = find(rpe>=edges(j) & rpe<edges(j+1));
        pRcurr = mean(pChoicePred(currInd) - (1-pChoicePred(currInd)), 'omitnan');
        pRnext = mean(stay(intersect(currInd+1, 1:length(choices))), 'omitnan');
        pRnext = pRnext - (1-pRnext);
        allMeans(i,j) = pRnext-pRcurr;
        meanPe(i,j) = mean(rpe(currInd), 'omitnan');
    end
    % allMeans(i,:) = allMeans(i,:) - meanChange;
